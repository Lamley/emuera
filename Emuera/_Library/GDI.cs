...
namespace MinorShift._Library
{
    ...
    internal static class GDI
    {
        ...
        private static IntPtr defaulthFont;
        private static IntPtr defaulthBrush;
        private static IntPtr defaulthPen;
        private static Color lastTextColor;
        private static Color lastBrushColor;
        private static Color lastPenColor;
        private static Size fontMetrics;
        private static bool usingStockBrush;
        ...
        private static extern int TabbedTextOutW(IntPtr hdc, int x, int y, string lpString, int nCount,
            int nTabPositions, ref int lpnTabStopPositions, int nTabOrigin);

        [DllImport("user32", EntryPoint = "GetTabbedTextExtent", CharSet = CharSet.Auto)]
        private static extern int GetTabbedTextExtentW(IntPtr hdc, string lpString, int nCount, int nTabPositions,
        ...
        [DllImport("gdi32.dll", ExactSpelling = true, PreserveSig = true, SetLastError = true)]
        private static extern bool Ellipse(IntPtr hdc, int nLeftRect, int nTopRect,
            int nRightRect, int nBottomRect);

        [DllImport("gdi32.dll")]
        private static extern bool Pie(IntPtr hdc, int nLeftRect, int nTopRect, int nRightRect,
        ...
        {
            hDC = g.GetHdc();
            var hBrush = CreateSolidBrush(ColorTranslator.ToWin32(backGroundColor));
            defaulthBrush = SelectObject(hDC, hBrush);
            var hPen = CreatePen(0, 0, ColorTranslator.ToWin32(backGroundColor));
        ...
        public static void SetPenColor(Color color)
        {
            if (lastPenColor == color)
                return;
            var hPen = CreatePen(0, 0, ColorTranslator.ToWin32(color));
        ...
        private static void TabbedTextOut98(string str, int x, int y)
        {
            //TextOut(hDC, p.X, p.Y, str, str.Length);
            TabbedTextOutW(hDC, x, y, str, LangManager.GetStrlenLang(str), 0, ref devnull, 0);
        }

        private static void TabbedTextOutNT(string str, int x, int y)
        {
            //TextOut(hDC, p.X, p.Y, str, str.Length);
            TabbedTextOutW(hDC, x, y, str, str.Length, 0, ref devnull, 0);
        }
        ...
        public static void DrawImage(int destX, int destY, int width, int height, IntPtr srchDC, Rectangle srcRect)
        {
            if (srcRect.Height == height && srcRect.Width == width)
                ...
                StretchBlt(hDC, destX, destY, width, height, srchDC, srcRect.X, srcRect.Y, srcRect.Width,
                    srcRect.Height, TernaryRasterOperations.SRCCOPY);
        }


        #region MesureText用

        private static IntPtr hDCMesureText;
        private static Font mtLastFont;
        private static IntPtr mtDefaulthFont;

        private static Size MeasureText98(string str, Font font)
        {
            if (mtLastFont != font)
            {
                var hFont = font.ToHfont();
                var hOldFont = SelectObject(hDCMesureText, hFont);
                if (mtLastFont == null)
            ...
            var ret = GetTabbedTextExtentW(hDCMesureText, str, LangManager.GetStrlenLang(str), 0, ref devnull);
            var size = new Size(ret & 0xffff, (ret >> 16) & 0xffff);
            return size;
        }

        private static Size MeasureTextNT(string str, Font font)
        {
            if (mtLastFont != font)
            {
                var hFont = font.ToHfont();
            ...
            var ret = GetTabbedTextExtentW(hDCMesureText, str, str.Length, 0, ref devnull);
            var size = new Size(ret & 0xffff, (ret >> 16) & 0xffff);
            return size;
        }
