...
namespace MinorShift.Emuera.GameProc
{
    ...
    internal enum SystemStateCode
    {
        ...
        Train_CallEventComEnd =
            0x19 | __CAN_BEGIN__, //@EVENTCOMENDの呼び出し中。スキップ可能。Train_CallEventTrainへ帰る。@USERCOMの呼び出し中もここ
    ...
    internal sealed class ProcessState
    {
        ...
        private BeginType begintype = BeginType.NULL;
        ...
        public LogicalLine CurrentLine { get; set; }
        ...
        public SystemStateCode SystemState { get; set; } = SystemStateCode.Title_Begin;
...
//TrimとToUpper済みのはず
        ...
        public void SetBegin(BeginType type)
        {
            ...
            switch (type)
            {
                ...
                case BeginType.FIRST:
                    if ((SystemState & SystemStateCode.__CAN_BEGIN__) != SystemStateCode.__CAN_BEGIN__)
                    {
                        errmes = "BEGIN";
                        goto err;
                    }
        ...
        {
            if (saveData)
                SystemState = SystemStateCode.SaveGame_Begin;
            else
                SystemState = SystemStateCode.LoadGame_Begin;
            //ClearFunctionList();
        }
        ...
        public void ClearFunctionList()
        {
            ...
            foreach (var called in functionList)
        ...
        public void Begin()
        {
            ...
            if (SystemState == SystemStateCode.Shop_CallEventShop)
                return;

            switch (begintype)
            {
                case BeginType.SHOP:
                    if (SystemState == SystemStateCode.Normal)
                    ...
                    break;
                case BeginType.TRAIN:
                    SystemState = SystemStateCode.Train_Begin;
                    break;
                case BeginType.AFTERTRAIN:
                    SystemState = SystemStateCode.AfterTrain_Begin;
                    break;
                case BeginType.ABLUP:
                    SystemState = SystemStateCode.Ablup_Begin;
                    break;
                case BeginType.TURNEND:
                    SystemState = SystemStateCode.Turnend_Begin;
                    break;
                case BeginType.FIRST:
                    SystemState = SystemStateCode.First_Begin;
                    break;
                case BeginType.TITLE:
...
//JUMPした場合。即座にRETURN RESULTする。
        ...
        public void IntoFunction(CalledFunction call, UserDefinedFunctionArgument srcArgs, ExpressionMediator exm)
        {
            ...
            if (srcArgs != null)
            {
                ...
                if (call.TopLabel.hasPrivDynamicVar)
                    call.TopLabel.In();
                //更新した変数へ引数を代入
                for (var i = 0; i < call.TopLabel.Arg.Length; i++)
        ...
        {
            var ret = new ProcessState(console);
            ret.IsClone = true;
            //どうせ消すからコピー不要
