...
namespace MinorShift.Emuera.GameProc
{
    internal sealed class ErbLoader
    {
        ...
        private readonly List<string> ignoredFNFWarningFileList = new List<string>();
        ...
        public bool LoadErbFiles(string erbDir, bool displayReport, LabelDictionary labelDictionary)
        {
            ...
            noError = true;
            var starttime = WinmmTimer.TickCount;
            try
            {
                labelDic.RemoveAll();
                for (var i = 0; i < erbFiles.Count; i++)
                {
                    var filename = erbFiles[i].Key;
                    var file = erbFiles[i].Value;
#if DEBUG
                    if (displayReport)
            ...
            try
            {
                ...
                while ((st = eReader.ReadEnabledLine()) != null)
                {
                    ...
                    if (st.Current == '[' && st.Next != '[')
                    {
                        ...
                        var token2 = LexicalAnalyzer.ReadSingleIdentifier(st);
                    ...
                    {
                        var isFunction = st.Current == '@';
                        nextLine = LogicalLineParser.ParseLabelLine(st, position, output);
                        if (isFunction)
                        {
                            var label = (FunctionLabelLine) nextLine;
                            lastLabelLine = label;
                            ...
                            else // if (label is FunctionLabelLine)
                            {
                                ...
                                {
                                    var seniorLabel = labelDic.GetSameNameLabel(label);
                                    if (seniorLabel != null)
                                    {
                                        //output.NewLine();
                                        ParserMediator.Warn(
                                            "関数@" + label.LabelName + "は既に定義(" + seniorLabel.Position.Filename + "の" +
                                            seniorLabel.Position.LineNo + "行目)されています", position, 1);
                                        funcCount = -1;
                                    }
                                }
                                funcCount++;
                                if (Program.AnalysisMode && Config.PrintCPerLine > 0 &&
                                    funcCount % Config.PrintCPerLine == 0)
                                {
                                    output.NewLine();
                                    output.PrintSystemLine("　");
                                }
                        ...
                        else
                        {
                            ...
                            {
                                var gotoLabel = (GotoLabelLine) nextLine;
                                gotoLabel.ParentLabelLine = lastLabelLine;
                                if (lastLabelLine != null && !labelDic.AddLabelDollar(gotoLabel))
                                {
                                    var pos = labelDic.GetLabelDollar(gotoLabel.LabelName, lastLabelLine).Position;
                                    ParserMediator.Warn(
                                        "ラベル名$" + gotoLabel.LabelName + "は既に同じ関数内(" + pos.Filename + "の" + pos.LineNo +
                                        "行目)で使用されています", position, 2);
                                }
                            }
                        }
        ...
        {
            var labelList = labelDic.GetAllLabels(false);
        ...
        {
            var wc = label.PopRowArgs();
            string errMes = null;
            var subNames = new SingleTerm[0];
            var args = new VariableTerm[0];
            var defs = new SingleTerm[0];
            var maxArg = -1;
            var maxArgs = -1;
            ...
            if (!wc.EOL)
            {
                ...
                if (symbol.Type == '[') //TODO:subNames 結局実装しないかも
                {
                    ...
                    {
                        errMes = "関数定義の[]内の引数は空にできません";
                        goto err;
                    }
                    subNames = new SingleTerm[subNamesRow.Length];
                    for (var i = 0; i < subNamesRow.Length; i++)
                    {
                        ...
                        var term = subNamesRow[i].Restructure(exm);
                        subNames[i] = term as SingleTerm;
                        if (subNames[i] == null)
                        {
                            errMes = "関数定義の[]内の引数は定数のみ指定できます";
                            goto err;
                        }
                    }
                ...
                if (!wc.EOL)
                {
                    ...
                    for (var i = 0; i < length; i++)
                    {
                        ...
                        if (!vTerm.Identifier.IsReference) //参照型なら添え字不要
                        {
                            ...
                            {
                                errMes = "関数定義の参照型でない引数\"" + vTerm.Identifier.Name + "\"に添え字が指定されていません";
                                goto err;
                            }
                            if (!vTerm.isAllConst)
                            {
                                errMes = "関数定義の引数の添え字には定数を指定してください";
                                goto err;
                            }
                        }
                        for (var j = 0; j < i; j++)
                            if (vTerm.checkSameTerm(args[j]))
                                ParserMediator.Warn(
                                    "第" + Strings.StrConv((i + 1).ToString(), VbStrConv.Wide, Config.Language) +
                                    "引数\"" + vTerm.GetFullString() + "\"はすでに第" +
                                    Strings.StrConv((j + 1).ToString(), VbStrConv.Wide, Config.Language) +
                                    "引数として宣言されています", label, 1, false, false);
                        ...
                        var canDef = vTerm.Identifier.Code == VariableCode.ARG ||
                                     vTerm.Identifier.Code == VariableCode.ARGS || vTerm.Identifier.IsPrivate;
                        term = argsRow[i * 2 + 1];
                        if (term is NullTerm)
                        {
                        ...
                        else
                        {
                            ...
                            if (!canDef)
                            {
                                errMes = "引数の初期値を定義できるのは\"ARG\"、\"ARGS\"またはプライベート変数のみです";
                                goto err;
                            }
                            if (vTerm.Identifier.IsReference)
                            {
                                errMes = "参照渡しの引数に初期値は定義できません";
                                goto err;
                            }
        ...
        private void checkScript()
        {
            ...
            var labelDepth = -1;
            var labelList = labelDic.GetAllLabels(true);
            ...
            labelDepth = -1;
            var ignoredFNCWarningFileList = new List<string>();
            var ignoredFNCWarningCount = 0;

            var ignoreAll = false;
            var notCalledWarning = Config.FunctionNotCalledWarning;
...
//callform系が使われたら全ての関数が呼び出されたとみなす。
            ...
            {
                var ignoreUncalledFunction = Config.IgnoreUncalledFunction;
                foreach (var label in labelList)
                {
                    if (label.Depth != labelDepth)
                        continue;
                    ...
                    {
                        var filename = label.Position.Filename.ToUpper();
            ...
            if (Program.AnalysisMode && (warningDic.Keys.Count > 0 || GlobalStatic.tempDic.Keys.Count > 0))
            {
                ...
                if (warningDic.Keys.Count > 0)
                {
                    ...
                    foreach (var labelName in warningDic.Keys)
                        output.PrintError("　　" + labelName + ": " + warningDic[labelName] + "回");
                }
                if (GlobalStatic.tempDic.Keys.Count > 0)
                {
                    output.PrintError("　○文中関数:");
                    foreach (var labelName in GlobalStatic.tempDic.Keys)
                        output.PrintError("　　" + labelName + ": " + GlobalStatic.tempDic[labelName] + "回");
                }
            }
            else
            {
                if (ignoredFNCWarningCount > 0 && Config.DisplayWarningLevel <= 1 &&
                    notCalledWarning != DisplayWarningFlag.IGNORE)
                    output.PrintError(string.Format("警告Lv1:定義された関数が一度も呼び出されていない事に関する警告を{0}件無視しました",
                        ignoredFNCWarningCount));
                if (ignoredFNFWarningCount > 0 && Config.DisplayWarningLevel <= 2 &&
                    notCalledWarning != DisplayWarningFlag.IGNORE)
                    output.PrintError(string.Format("警告Lv2:定義されていない関数を呼び出した事に関する警告を{0}件無視しました",
                        ignoredFNFWarningCount));
            }
            ParserMediator.FlushWarningList();
            if (Config.DisplayReport)
                output.PrintError(string.Format("非コメント行数:{0}, 全関数合計:{1}, 被呼出関数合計:{2}", enabledLineCount, labelDic.Count,
                    usedLabelCount));
            if (Config.AllowFunctionOverloading && Config.WarnFunctionOverloading)
            {
                var overloadedList = GlobalStatic.IdentifierDictionary.GetOverloadedList(labelDic);
                if (overloadedList.Count > 0)
                {
                    output.NewLine();
                    output.PrintError("＊＊＊＊＊警告＊＊＊＊＊");
                    foreach (var funcname in overloadedList)
        ...
        private void printFunctionNotFoundWarning(string str, LogicalLine line, int level, bool isError)
        {
            ...
            if (level < Config.DisplayWarningLevel)
                return;
            var ignore = false;
            var warnFlag = Config.FunctionNotFoundWarning;
            ...
            {
                var filename = line.Position.Filename.ToUpper();
                if (!string.IsNullOrEmpty(filename))
...
//ここでエラーを捕まえることは本来はないはず。ExeEE相当。
            ...
            catch (Exception exc)
            {
                ...
                var errmes = exc is EmueraException ? exc.Message : exc.GetType() + ":" + exc.Message;
                ParserMediator.Warn("@" + label.LabelName + " の解析中にエラー:" + errmes, label, 2, true, false,
                    !(exc is EmueraException) ? exc.StackTrace : null);
                label.ErrMes = "ロード時に解析に失敗した関数が呼び出されました";
            }
            finally
            {
        ...
        private void nestCheck(FunctionLabelLine label)
        {
            ...
            //PRINTDATA系もここでチェック
            LogicalLine nextLine = label;
            var tempLineList = new List<InstructionLine>();
            var nestStack = new Stack<InstructionLine>();
            var SelectcaseStack = new Stack<InstructionLine>();
            ...
            while (true)
            {
                ...
                if (!(nextLine is InstructionLine))
                {
                    if (nextLine is GotoLabelLine)
                    {
                        var currentBaseFunc = nestStack.Count == 0 ? null : nestStack.Peek();
                        if (currentBaseFunc != null)
                            if (currentBaseFunc.FunctionCode == FunctionCode.PRINTDATA
                                ...
                                ParserMediator.Warn(currentBaseFunc.Function.Name + "構文中に$ラベルを定義することはできません", nextLine,
                                    2, true, false);
                    }
                    continue;
                }
                var func = (InstructionLine) nextLine;
                pairLine = null;
                ...
                if (baseFunc != null)
                    ...
                    {
                        if (func.FunctionCode != FunctionCode.DATA && func.FunctionCode != FunctionCode.DATAFORM &&
                            func.FunctionCode != FunctionCode.DATALIST
                            && func.FunctionCode != FunctionCode.ENDLIST && func.FunctionCode != FunctionCode.ENDDATA)
                        {
                            ParserMediator.Warn(
                                baseFunc.Function.Name + "構文に使用できない命令\'" + func.Function.Name + "\'が含まれています", func, 2,
                                true, false);
                            continue;
                        }
                    }
                    ...
                    {
                        if (func.FunctionCode != FunctionCode.DATA && func.FunctionCode != FunctionCode.DATAFORM &&
                            func.FunctionCode != FunctionCode.ENDLIST)
                        {
                            ParserMediator.Warn("DATALIST構文に使用できない命令\'" + func.Function.Name + "\'が含まれています", func, 2,
                                true, false);
                            continue;
                        }
                    }
                    else if (baseFunc.FunctionCode == FunctionCode.TRYCALLLIST ||
                             baseFunc.FunctionCode == FunctionCode.TRYJUMPLIST ||
                             baseFunc.FunctionCode == FunctionCode.TRYGOTOLIST)
                    {
                    ...
                    {
                        if (baseFunc.IfCaseList.Count == 0 && func.FunctionCode != FunctionCode.CASE &&
                            func.FunctionCode != FunctionCode.CASEELSE && func.FunctionCode != FunctionCode.ENDSELECT)
                        {
                            ParserMediator.Warn("SELECTCASE構文の分岐の外に命令\'" + func.Function.Name + "\'が含まれています", func, 2,
                                true, false);
                            continue;
                        }
                    }
                switch (func.FunctionCode)
                {
                    ...
                    case FunctionCode.CONTINUE:
                        ...
                        if (pairLine == null)
                        {
                            ParserMediator.Warn("REPEAT, FOR, WHILE, DOの中以外で" + func.Function.Name + "文が使われました", func,
                                2, true, false);
                            break;
                        }
                    ...
                    {
                        var selectLine = nestStack.Count == 0 ? null : nestStack.Peek();
                        if (selectLine == null || selectLine.FunctionCode != FunctionCode.SELECTCASE &&
                            SelectcaseStack.Count == 0)
                        {
                            ParserMediator.Warn("SELECTCASE～ENDSELECTの外で" + func.Function.Name + "文が使われました", func, 2,
                                true, false);
                            break;
                        }
                        if (selectLine.FunctionCode != FunctionCode.SELECTCASE && SelectcaseStack.Count > 0)
                        {
                            ...
                            {
                                ParserMediator.Warn(
                                    selectLine.Function.Name + "文に対応する" +
                                    FunctionIdentifier.getMatchFunction(selectLine.FunctionCode) + "がない状態で" +
                                    func.Function.Name + "文に到達しました", func, 2, true, false);
                                ...
                                selectLine =
                                    nestStack.Count == 0
                                        ? null
                                        : nestStack.Peek(); //ちなみにnullになることはない（SELECTCASEがない場合は上で弾けるから）
                            } while (selectLine != null && selectLine.FunctionCode != FunctionCode.SELECTCASE);
                            break;
                        }
                        if (selectLine.IfCaseList.Count > 0 &&
                            selectLine.IfCaseList[selectLine.IfCaseList.Count - 1].FunctionCode ==
                            FunctionCode.CASEELSE)
                            ParserMediator.Warn("CASEELSE文より後で" + func.Function.Name + "文が使われました", func, 1, false,
                                false);
                    ...
                    case FunctionCode.ENDSELECT:
                    {
                        ...
                        if (selectLine.IsError)
                            break;
                        var term = ((ExpressionArgument) selectLine.Argument).Term;
                        if (term == null)
                        {
                        ...
                        foreach (var caseLine in selectLine.IfCaseList)
                        {
                            ...
                            if (caseLine.FunctionCode == FunctionCode.CASEELSE)
                                continue;
                            var caseExps = ((CaseArgument) caseLine.Argument).CaseExps;
                    ...
                    case FunctionCode.LOOP:
                        var parentFunc = FunctionIdentifier.getParentFunc(func.FunctionCode);
                        //if (parentFunc == FunctionCode.__NULL__)
                        //    throw new ExeEE("何か変？");
                        if (nestStack.Count == 0
                            || nestStack.Peek().FunctionCode != parentFunc)
                        {
                            ParserMediator.Warn("対応する" + parentFunc + "の無い" + func.Function.Name + "文です", func, 2, true,
                                false);
                            break;
                        }
                    ...
                    case FunctionCode.DATALIST:
                    {
                        ...
                        if (pline == null ||
                            !pline.Function.IsPrintData() && pline.FunctionCode != FunctionCode.STRDATA)
                        {
                    ...
                    case FunctionCode.ENDLIST:
                    {
                        ...
                        if (tempLineList.Count == 0)
                            ParserMediator.Warn("DATALIST命令に表示データが与えられていません（このDATALISTは空文字列を表示します）", func, 1, false,
                                false);
                        nestStack.Pop();
                    ...
                    {
                        var pdata = nestStack.Count == 0 ? null : nestStack.Peek();
                        if (pdata == null || !pdata.Function.IsPrintData() &&
                            pdata.FunctionCode != FunctionCode.DATALIST && pdata.FunctionCode != FunctionCode.STRDATA)
                        {
                            ParserMediator.Warn("対応するPRINTDATA系命令のない" + func.Function.Name + "です", func, 2, true,
                                false);
                            break;
                        }
                        var iList = new List<InstructionLine>();
                    ...
                    case FunctionCode.ENDDATA:
                    {
                        ...
                        if (pline == null ||
                            !pline.Function.IsPrintData() && pline.FunctionCode != FunctionCode.STRDATA)
                        {
                            ParserMediator.Warn("対応するPRINTDATA系命令もしくはSTRDATAのない" + func.Function.Name + "です", func, 2,
                                true, false);
                            break;
                        }
                        ...
                        if (pline.dataList.Count == 0)
                            ParserMediator.Warn(pline.Function.Name + "命令に表示データがありません（この命令は無視されます）", func, 1, false,
                                false);
                        pline.JumpTo = func;
                        nestStack.Pop();
                        break;
                    ...
                    case FunctionCode.TRYGOTOLIST:
                        foreach (var iLine in nestStack)
                            if (iLine.FunctionCode == FunctionCode.TRYCALLLIST ||
                                iLine.FunctionCode == FunctionCode.TRYJUMPLIST ||
                                iLine.FunctionCode == FunctionCode.TRYGOTOLIST)
                            {
                    ...
                    case FunctionCode.FUNC:
                    {
                        ...
                        if (pFunc == null ||
                            pFunc.FunctionCode != FunctionCode.TRYCALLLIST &&
                            pFunc.FunctionCode != FunctionCode.TRYJUMPLIST &&
                            pFunc.FunctionCode != FunctionCode.TRYGOTOLIST)
                        {
                            ParserMediator.Warn("対応するTRYCALLLIST系命令のない" + func.Function.Name + "です", func, 2, true,
                                false);
                            break;
                        }
                        if (func.Argument == null)
                        {
                            ParserMediator.Warn("TRYCALLLIST系命令中に無効な" + func.Function.Name + "が存在します", pFunc, 2, true,
                                false);
                            break;
                        }
                    ...
                    case FunctionCode.ENDFUNC:
                        ...
                        if (pf == null ||
                            pf.FunctionCode != FunctionCode.TRYCALLLIST &&
                            pf.FunctionCode != FunctionCode.TRYJUMPLIST && pf.FunctionCode != FunctionCode.TRYGOTOLIST)
                        {
                            ParserMediator.Warn("対応するTRYCALLLIST系命令のない" + func.Function.Name + "です", func, 2, true,
                                false);
                            break;
            ...
            while (nestStack.Count != 0)
            {
                ...
                var funcName = func.Function.Name;
                var funcMatch = FunctionIdentifier.getMatchFunction(func.FunctionCode);
                if (func != null)
            ...
            while (true)
            {
                ...
                if (func.Function.Instruction != null)
                {
                    ...
                    if (FunctionNotFoundName != null)
                        if (!Program.AnalysisMode)
                            printFunctionNotFoundWarning("指定された関数名\"@" + FunctionNotFoundName + "\"は存在しません", func, 2,
                                true);
                        else
        ...
        private sealed class PPState
        {
            ...
            private readonly Stack<bool> disabledStack = new Stack<bool>();
            private bool done;
            private readonly Stack<bool> doneStack = new Stack<bool>();
            private readonly Stack<string> ppMatch = new Stack<string>();
            private bool skip;

            internal void AddKeyWord(string token, string token2, ScriptPosition position)
            {
                var token2enabled = string.IsNullOrEmpty(token2);
                switch (token)
                {
                    case "SKIPSTART":
                    ...
                    case "SKIPEND":
                    {
                        ...
                        var match = ppMatch.Count == 0 ? "" : ppMatch.Pop();
                        if (match != "SKIPEND")
                        {
