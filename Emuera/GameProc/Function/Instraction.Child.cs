    ...
    internal sealed partial class FunctionIdentifier
    {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                else
                {
                    ...
                    if (isForms)
                    {
                        ...
                        var wt = LexicalAnalyzer.AnalyseFormattedString(new StringStream(str), FormStrEndWith.EoL,
                            false);
        ...
        private sealed class PRINT_RECT_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                for (var i = 0; i < intExpArg.TermList.Length; i++)
                    param[i] = toUInt32inArg(intExpArg.TermList[i].GetIntValue(exm), "DELDATA", i + 1);
        ...
        private sealed class PRINT_SPACE_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                var param32 = toUInt32inArg(param, "DELDATA", 1);
        ...
        private sealed class METHOD_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var term = ((MethodArgument) func.Argument).MethodTerm;
                var type = term.GetOperandType();
        ...
        private sealed class SET_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                {
                    var arg = (SpSetArrayArgument) func.Argument;
                    if (arg.VariableDest.IsInteger)
                    {
                        ...
                        {
                            var values = new long[arg.TermList.Length];
                    ...
                    else
                    {
                        ...
                        {
                            var values = new string[arg.TermList.Length];
                            for (var i = 0; i < values.Length; i++)
                ...
                var spsetarg = (SpSetArgument) func.Argument;
                if (spsetarg.VariableDest.IsInteger)
                {
                    var src = spsetarg.IsConst ? spsetarg.ConstInt : spsetarg.Term.GetIntValue(exm);
                ...
                {
                    var src = spsetarg.IsConst ? spsetarg.ConstStr : spsetarg.Term.GetStrValue(exm);
                    spsetarg.VariableDest.SetValue(src, exm);
                }
            }
        }
        ...
        private sealed class REUSELASTLINE_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var term = ((ExpressionArgument) func.Argument).Term;
        ...
        private sealed class CLEARLINE_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                var delNum = (int) intExpArg.Term.GetIntValue(exm);
        ...
        private sealed class SETBIT_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var spsetarg = (BitArgument) func.Argument;
                var varTerm = spsetarg.VariableDest;
                var terms = spsetarg.Term;
                for (var i = 0; i < terms.Length; i++)
                {
                    var x = terms[i].GetIntValue(exm);
                    if (x < 0 || x > 63)
        ...
        private sealed class TWAIT_Instruction : AbstractInstruction
        {
            ...
            {
                exm.Console.ReadAnyKey();
                var arg = (SpSwapCharaArgument) func.Argument;
                var time = arg.X.GetIntValue(exm);
                var flag = arg.Y.GetIntValue(exm);
        ...
        private sealed class INPUT_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var arg = (ExpressionArgument) func.Argument;
                var req = new InputRequest();
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                if (arg.Term != null)
                {
                    ...
                    if (arg.IsConst)
                        ...
                        def = arg.Term.GetIntValue(exm);
                    if (def > 9)
                        def = long.Parse(def.ToString().Remove(1));
                    if (def >= 0)
                    {
                        req.HasDefValue = true;
                        req.DefIntValue = def;
        ...
        private sealed class TINPUT_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var tinputarg = (SpTInputsArgument) func.Argument;

                var req = new InputRequest();
                req.InputType = InputType.IntValue;
                req.HasDefValue = true;
                req.OneInput = isOne;
                var x = tinputarg.Time.GetIntValue(exm);
                var y = tinputarg.Def.GetIntValue(exm);
                ...
                var z = tinputarg.Disp != null ? tinputarg.Disp.GetIntValue(exm) : 1;
                req.Timelimit = x;
                req.DefIntValue = y;
                req.DisplayTime = z != 0;
        ...
        private sealed class TINPUTS_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                req.OneInput = isOne;
                var x = tinputarg.Time.GetIntValue(exm);
                var strs = tinputarg.Def.GetStrValue(exm);
        ...
        private sealed class CALLF_Instruction : AbstractInstruction
        {
            ...
            public override void SetJumpTo(ref bool useCallForm, InstructionLine func, int currentDepth,
                ref string FunctionoNotFoundName)
            {
                if (!func.Argument.IsConst)
                {
                    useCallForm = true;
                ...
                var callfArg = (SpCallFArgment) func.Argument;
                if (Config.ICFunction)
                    callfArg.ConstStr = callfArg.ConstStr.ToUpper();
                try
                {
                    callfArg.FuncTerm =
                        GlobalStatic.IdentifierDictionary.GetFunctionMethod(GlobalStatic.LabelDictionary,
                            callfArg.ConstStr, callfArg.RowArgs, true);
                }
                catch (CodeEE e)
                {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                {
                    var spCallformArg = (SpCallFArgment) func.Argument;
                    labelName = spCallformArg.FuncnameTerm.GetStrValue(exm);
                    mToken = GlobalStatic.IdentifierDictionary.GetFunctionMethod(GlobalStatic.LabelDictionary,
                        labelName, spCallformArg.RowArgs, true);
                }
                else
                {
        ...
        private sealed class BAR_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var barArg = (SpBarArgument) func.Argument;
                var var = barArg.Terms[0].GetIntValue(exm);
                var max = barArg.Terms[1].GetIntValue(exm);
                var length = barArg.Terms[2].GetIntValue(exm);
        ...
        private sealed class TIMES_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var timesArg = (SpTimesArgument) func.Argument;
                var var = timesArg.VariableDest;
                if (Config.TimesNotRigorousCalculation)
                {
                    var d = var.GetIntValue(exm) * timesArg.DoubleValue;
                    unchecked
                    {
                ...
                {
                    var d = var.GetIntValue(exm) * (decimal) timesArg.DoubleValue;
                    unchecked
                    {
        ...
        private sealed class ADDCHARA_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                var intExpArg = (ExpressionArrayArgument) func.Argument;
                long integer = -1;
                var charaNoList = new long[intExpArg.TermList.Length];
                var i = 0;
                foreach (var int64Term in intExpArg.TermList)
                {
        ...
        private sealed class SWAPCHARA_Instruction : AbstractInstruction
        {
            ...
            {
                var arg = (SpSwapCharaArgument) func.Argument;
                var x = arg.X.GetIntValue(exm);
                var y = arg.Y.GetIntValue(exm);
        ...
        private sealed class SORTCHARA_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var spSortArg = (SpSortcharaArgument) func.Argument;
                long elem = 0;
                var sortKey = spSortArg.SortKey;
        ...
        private sealed class VARSET_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var spvarsetarg = (SpVarSetArgument) func.Argument;
                var var = spvarsetarg.VariableDest;
                var p = var.GetFixedVariableTerm(exm);
                var start = 0;
                var end = 0;
                //endを先に取って判定の処理変更
                ...
                if (var.IsString)
                {
                    var src = spvarsetarg.Term.GetStrValue(exm);
                    exm.VEvaluator.SetValueAll(p, src, start, end);
                }
                else
                {
                    var src = spvarsetarg.Term.GetIntValue(exm);
                    exm.VEvaluator.SetValueAll(p, src, start, end);
                }
            }
        }
        ...
        private sealed class CVARSET_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var spvarsetarg = (SpCVarSetArgument) func.Argument;
                var p = spvarsetarg.VariableDest.GetFixedVariableTerm(exm);
                var index = spvarsetarg.Index.GetValue(exm);
                var charaNum = (int) exm.VEvaluator.CHARANUM;
                var start = 0;
                if (spvarsetarg.Start != null)
                {
                    start = (int) spvarsetarg.Start.GetIntValue(exm);
                    if (start < 0 || start >= charaNum)
                        throw new CodeEE("命令CVARSETの第４引数(" + start + ")がキャラクタの範囲外です");
                }
                var end = 0;
                if (spvarsetarg.End != null)
                {
                    ...
                    if (end < 0 || end > charaNum)
                        throw new CodeEE("命令CVARSETの第５引数(" + end + ")がキャラクタの範囲外です");
                }
                else
                {
                    end = charaNum;
        ...
        private sealed class SAVECHARA_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var arg = (ExpressionArrayArgument) func.Argument;
                var terms = arg.TermList;
                var datFilename = terms[0].GetStrValue(exm);
                var savMes = terms[1].GetStrValue(exm);
                var savCharaList = new int[terms.Length - 2];
                var charanum = (int) exm.VEvaluator.CHARANUM;
                for (var i = 0; i < savCharaList.Length; i++)
                {
                    var v = terms[i + 2].GetIntValue(exm);
        ...
        private sealed class DELDATA_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                var target32 = toUInt32inArg(target, "DELDATA", 1);
        ...
        private sealed class REF_Instruction : AbstractInstruction
        {
            ...
            {
                throw new NotImplCodeEE();
                var arg = (RefArgument) func.Argument;
                string str = null;
                if (arg.SrcTerm != null)
                    str = arg.SrcTerm.GetStrValue(exm);
                if (arg.RefMethodToken != null)
                {
                    var srcRef = arg.SrcRefMethodToken;
                    var call = arg.SrcCalledFunction;
                    if (str != null) //REFBYNAMEかつ第二引数が定数でない
                    {
                        srcRef = GlobalStatic.IdentifierDictionary.GetRefMethod(str);
                        if (srcRef == null)
                        {
                            var label = GlobalStatic.LabelDictionary.GetNonEventLabel(str);
                            //if (label == null)
                ...
                var refVar = arg.RefVarToken;
                var srcVar = arg.SrcVarToken;
                string errmes;
                if (str != null)
        ...
        private sealed class TOOLTIP_SETCOLOR_Instruction : AbstractInstruction
        {
            ...
            {
                var arg = (SpSwapCharaArgument) func.Argument;
                var foreColor = arg.X.GetIntValue(exm);
                var backColor = arg.Y.GetIntValue(exm);
                ...
                var fc = Color.FromArgb((int) foreColor >> 16, ((int) foreColor >> 8) & 0xFF, (int) foreColor & 0xFF);
                var bc = Color.FromArgb((int) backColor >> 16, ((int) backColor >> 8) & 0xFF, (int) backColor & 0xFF);
        ...
        private sealed class REPEAT_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var forArg = (SpForNextArgment) func.Argument;
        ...
        private sealed class WHILE_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var expArg = (ExpressionArgument) func.Argument;
        ...
        private sealed class SIF_Instruction : AbstractInstruction
        {
                ...
                ref string FunctionoNotFoundName)
            {
                ...
                {
                    var sifFunc = (InstructionLine) jumpto;
        ...
        private sealed class IF_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var ifJumpto = func.JumpTo; //ENDIF
                //チェック済み
                ...
                for (var i = 0; i < func.IfCaseList.Count; i++)
                {
                    ...
                    var value = ((ExpressionArgument) line.Argument).Term.GetIntValue(exm);
                    if (value != 0) //式が真
                    {
                        ifJumpto = line;
                        break;
        ...
        private sealed class SELECTCASE_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var caseJumpto = func.JumpTo; //ENDSELECT
                var selectValue = ((ExpressionArgument) func.Argument).Term;
                string sValue = null;
                long iValue = 0;
                ...
                for (var i = 0; i < func.IfCaseList.Count; i++)
                {
                    ...
                    var caseArg = (CaseArgument) line.Argument;
                    //チェック済み
                    //if (caseArg == null)
        ...
        private sealed class RETURNFORM_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                //    return;
                //int termnum = 0;
                var aSt = new StringStream(((ExpressionArgument) func.Argument).Term.GetStrValue(exm));
                var termList = new List<long>();
                while (!aSt.EOS)
                {
                    var wc = LexicalAnalyzer.Analyse(aSt, LexEndWith.Comma, LexAnalyzeFlag.None);
                    //exm.VEvaluator.SetResultX(ExpressionParser.ReduceIntegerTerm(wc, TermEndWith.EoL).GetIntValue(exm), termnum++);
        ...
        private sealed class RETURN_Instruction : AbstractInstruction
        {
            ...
            {
                //int termnum = 0;
                var expArrayArg = (ExpressionArrayArgument) func.Argument;
                ...
                var termList = new List<long>();
                foreach (var term in expArrayArg.TermList)
        ...
        private sealed class BREAK_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                var jumpTo = (InstructionLine) func.JumpTo;
                var iLine = (InstructionLine) jumpTo.JumpTo;
        ...
        private sealed class CONTINUE_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                if (jumpTo.FunctionCode == FunctionCode.REPEAT || jumpTo.FunctionCode == FunctionCode.FOR)
                {
                    ...
                    jumpTo.LoopCounter.PlusValue(jumpTo.LoopStep, exm);
                    var counter = jumpTo.LoopCounter.GetIntValue(exm);
                    //まだ回数が残っているなら、
                ...
                if (jumpTo.FunctionCode == FunctionCode.DO)
                {
                    ...
                    var tFunc = (InstructionLine) ((InstructionLine) func.JumpTo).JumpTo; //LOOP
                    if (tFunc.IsError)
                        throw new CodeEE(tFunc.ErrMes, tFunc.Position);
                    var expArg = (ExpressionArgument) tFunc.Argument;
                    if (expArg.Term.GetIntValue(exm) != 0) //式が真
        ...
        private sealed class RETURNF_Instruction : AbstractInstruction
        {
            ...
            public override void SetJumpTo(ref bool useCallForm, InstructionLine func, int currentDepth,
                ref string FunctionoNotFoundName)
            {
                var label = func.ParentLabelLine;
                if (!label.IsMethod)
        ...
        private sealed class CALL_Instruction : AbstractInstruction
        {
                ...
                ref string FunctionoNotFoundName)
            {
                ...
                var callArg = (SpCallArgment) func.Argument;
                var labelName = callArg.ConstStr;
                if (Config.ICFunction)
                    labelName = labelName.ToUpper();
                var call = CalledFunction.CallFunction(GlobalStatic.Process, labelName, func);
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                var spCallArg = (SpCallArgment) func.Argument;
        ...
        private sealed class CALLEVENT_Instruction : AbstractInstruction
        {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
            {
                ...
                if (Config.ICFunction)
                    labelName = labelName.ToUpper();
                var call = CalledFunction.CallEventFunction(GlobalStatic.Process, labelName, func);
                if (call == null)
                    return;
