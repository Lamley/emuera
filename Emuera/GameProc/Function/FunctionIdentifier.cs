...
namespace MinorShift.Emuera.GameProc.Function
{
    ...
    internal sealed partial class FunctionIdentifier
    {
        ...
        private static readonly Dictionary<string, FunctionIdentifier> funcDic =
            new Dictionary<string, FunctionIdentifier>();

        private static readonly Dictionary<FunctionCode, string> funcMatch = new Dictionary<FunctionCode, string>();

        private static readonly Dictionary<FunctionCode, FunctionCode> funcParent =
            new Dictionary<FunctionCode, FunctionCode>();
        ...
        static FunctionIdentifier()
        {
            ...
            addFunction(FunctionCode.RETURN,
                new RETURN_Instruction()); //関数の終了。RESULTに整数を格納可能。省略した場合、０。(次の@～～がRETURNと見なされる。)  
            addFunction(FunctionCode.RETURNFORM,
                new RETURNFORM_Instruction()); //関数の終了。RESULTに整数を格納可能。省略した場合、０。(次の@～～がRETURNと見なされる。)  
