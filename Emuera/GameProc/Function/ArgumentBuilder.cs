...
namespace MinorShift.Emuera.GameProc.Function
{
    internal abstract class ArgumentBuilder
    {
        ...
        {
            var varTerm = terms[i - 1] as VariableTerm;
            if (varTerm == null)
            {
        ...
        {
            var st = line.PopArgumentPrimitive();
    ...
    internal static partial class ArgumentParser
    {
        private static readonly Dictionary<FunctionArgType, ArgumentBuilder> argb =
            new Dictionary<FunctionArgType, ArgumentBuilder>();
        ...
        private sealed class SP_PRINTV_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                var st = line.PopArgumentPrimitive();
                var wc = LexicalAnalyzer.Analyse(st, LexEndWith.EoL, LexAnalyzeFlag.AnalyzePrintV);
                var args = ExpressionParser.ReduceArguments(wc, ArgsEndWith.EoL, false);
                for (var i = 0; i < args.Length; i++)
                    if (args[i] == null)
                    {
                        warn("引数を省略することはできません", line, 2, false);
                        return null;
                    }
                    else
                    {
        ...
        private sealed class SP_TIMES_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                var st = line.PopArgumentPrimitive();
                var wc = LexicalAnalyzer.Analyse(st, LexEndWith.Comma, LexAnalyzeFlag.None);
                st.ShiftNext();
                if (st.EOS)
                {
                    warn("引数が足りません", line, 2, false);
                ...
                var varTerm = term.Restructure(exm) as VariableTerm;
                if (varTerm == null)
                {
                    warn("第１引数に変数以外を指定することはできません", line, 2, false);
                    return null;
                }
                if (varTerm.IsString)
                {
                    warn("第１引数を文字列変数にすることはできません", line, 2, false);
                    return null;
                }
                if (varTerm.Identifier.IsConst)
                {
                    warn("第１引数に変更できない変数を指定することはできません", line, 2, false);
                    return null;
                }
        ...
        private sealed class FORM_STR_ANY_ArgumentBuilder : ArgumentBuilder
        {
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                while (true)
                {
                    var sfwt = LexicalAnalyzer.AnalyseFormattedString(st, FormStrEndWith.Comma, false);
                    var term = ExpressionParser.ToStrFormTerm(sfwt);
        ...
        private sealed class STR_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (line.FunctionCode == FunctionCode.SETCOLORBYNAME ||
                    line.FunctionCode == FunctionCode.SETBGCOLORBYNAME)
                {
                    var c = Color.FromName(rowStr);
                    if (c.A == 0)
                    {
        ...
        private sealed class FORM_STR_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                var sfwt = LexicalAnalyzer.AnalyseFormattedString(st, FormStrEndWith.EoL, false);
                var term = ExpressionParser.ToStrFormTerm(sfwt);
                term = term.Restructure(exm);
        ...
        private sealed class SP_VAR_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                var st = line.PopArgumentPrimitive();
                var iw = LexicalAnalyzer.ReadSingleIdentifierWord(st);
                if (iw == null)
                {
                    warn("第１引数を読み取ることができません", line, 2, false);
                    return null;
                }
                var idStr = iw.Code;
                var id = GlobalStatic.IdentifierDictionary.GetVariableToken(idStr, null, true);
                if (id == null)
                {
                    warn("第１引数に変数以外を指定することはできません", line, 2, false);
                    return null;
                ...
                {
                    warn("第１引数に配列でない変数を指定することはできません", line, 2, false);
                    return null;
                }
                LexicalAnalyzer.SkipWhiteSpace(st);
                if (!st.EOS)
        ...
        private sealed class SP_SORTCHARA_ArgumentBuilder : ArgumentBuilder
        {
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                var varTerm = new VariableTerm(GlobalStatic.VariableData.GetSystemVariableToken("NO"),
                    new IOperandTerm[] {new SingleTerm(0)});
                ...
                {
                    var term = ExpressionParser.ReduceExpressionTerm(wc, TermEndWith.Comma);
                    if (term == null)
                    {
                        warn("書式が間違っています", line, 2, false);
                        return null;
                    ...
                    {
                        warn("第１引数はキャラクタ変数でなければなりません", line, 2, false);
                        return null;
                    }
                    wc.ShiftNext();
                    if (!wc.EOL)
        ...
        private sealed class SP_SORT_ARRAY_ArgumentBuilder : ArgumentBuilder
        {
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (varTerm.Identifier.IsConst)
                {
                    warn("第１引数が変更できない変数です", line, 2, false);
                    return null;
                }
                if (!varTerm.Identifier.IsArray1D)
                {
                    warn("第１引数に１次元配列もしくは配列型キャラクタ変数以外を指定することはできません", line, 2, false);
                    return null;
                }
                ...
                {
                    warn("第２引数にソート方法指定子（FORWARD or BACK）以外が指定されています", line, 2, false);
                    return null;
                }
                ...
                if (id != null)
                {
                    ...
                    if (!wc.EOL)
                    {
                        ...
                        if (term3 == null)
                        {
                            warn("第３引数が解釈出来ません", line, 2, false);
                            return null;
                        }
                        if (!term3.IsInteger)
                        {
                            warn("第３引数が数値ではありません", line, 2, false);
                            return null;
                        }
                        wc.ShiftNext();
                        if (!wc.EOL)
                        {
                            ...
                            if (term4 == null)
                            {
                                warn("第４引数が解釈出来ません", line, 2, false);
                                return null;
                            }
                            if (!term4.IsInteger)
                            {
                                warn("第４引数が数値ではありません", line, 2, false);
                                return null;
                            }
                            wc.ShiftNext();
                            if (!wc.EOL)
        ...
        private sealed class SP_CALL_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (form)
                {
                    var sfw = LexicalAnalyzer.AnalyseFormattedString(st,
                        FormStrEndWith.LeftParenthesis_Bracket_Comma_Semicolon, true);
                    funcname = ExpressionParser.ToStrFormTerm(sfw);
                    funcname = funcname.Restructure(exm);
                }
                else
                {
                    var str = LexicalAnalyzer.ReadString(st, StrEndWith.LeftParenthesis_Bracket_Comma_Semicolon);
                    str = str.Trim(' ', '\t');
        ...
        private sealed class CASE_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                var wc = popWords(line);
                var args = ExpressionParser.ReduceCaseExpressions(wc);
        ...
        private sealed class SP_SET_ArgumentBuilder : ArgumentBuilder
        {
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                var destWc = line.PopAssignmentDestStr();
                ...
                {
                    assignwarn("代入文の左辺の読み取りに失敗しました", line, 2, false);
                    return null;
                }
                if (destTerms.Length != 1)
                {
                    assignwarn("代入文の左辺に余分な','があります", line, 2, false);
                    return null;
                }
                var varTerm = destTerms[0] as VariableTerm;
                if (varTerm == null)
                {
//
                ...
                if (st == null)
                    st = new StringStream("");
                var op = line.AssignOperator;
                IOperandTerm src = null;
                if (varTerm.IsInteger)
                {
                    if (op == OperatorCode.AssignmentStr)
                    {
                        assignwarn("整数型の代入に演算子" + OperatorManager.ToOperatorString(op) + "は使用できません", line, 2, false);
                        return null;
                    }
                    if (op == OperatorCode.Increment || op == OperatorCode.Decrement)
                    {
                        ...
                        if (!st.EOS)
                            ...
                            {
                                assignwarn("インクリメント行でインクリメント以外の処理が定義されています", line, 2, false);
                                return null;
                            }
                            else
                            {
                                assignwarn("デクリメント行でデクリメント以外の処理が定義されています", line, 2, false);
                                return null;
                            }
                    ...
                    var srcWc = LexicalAnalyzer.Analyse(st, LexEndWith.EoL, LexAnalyzeFlag.None);
                    var srcTerms = ExpressionParser.ReduceArguments(srcWc, ArgsEndWith.EoL, false);

                    if (srcTerms.Length == 0 || srcTerms[0] == null)
                    {
                        assignwarn("代入文の右辺の読み取りに失敗しました", line, 2, false);
                        return null;
                    }
                    if (srcTerms.Length != 1)
                    {
                        ...
                        {
                            assignwarn("複合代入演算では右辺に複数の値を含めることはできません", line, 2, false);
                            return null;
                        }
                        var allConst = true;
                        var constValues = new long[srcTerms.Length];
                        for (var i = 0; i < srcTerms.Length; i++)
                        {
                            if (srcTerms[i] == null)
                            {
                                assignwarn("代入式の右辺の値は省略できません", line, 2, false);
                                return null;
                            }
                            if (!srcTerms[i].IsInteger)
                            {
                                assignwarn("数値型変数に文字列は代入できません", line, 2, false);
                                return null;
                            }
                ...
                if (op == OperatorCode.Assignment)
                {
                    ...
                    var sfwt = LexicalAnalyzer.AnalyseFormattedString(st, FormStrEndWith.EoL, true);
                    var term = ExpressionParser.ToStrFormTerm(sfwt);
                ...
                if (op == OperatorCode.Mult || op == OperatorCode.Plus || op == OperatorCode.AssignmentStr)
                {
                    ...
                    if (op == OperatorCode.AssignmentStr)
                    {
                        if (srcTerms.Length == 1)
                        {
                            if (srcTerms[0].IsInteger)
                            {
                                assignwarn("文字列変数に数値型は代入できません", line, 2, false);
                                return null;
                            }
                            src = srcTerms[0].Restructure(exm);
                            ret = new SpSetArgument(varTerm, src);
                        ...
                        var allConst = true;
                        var constValues = new string[srcTerms.Length];
                        for (var i = 0; i < srcTerms.Length; i++)
                        {
                            if (srcTerms[i] == null)
                            {
                    ...
                    if (srcTerms.Length != 1)
                    {
                        assignwarn("代入文の右辺に余分な','があります", line, 2, false);
                        return null;
                    }
        ...
        private sealed class METHOD_ArgumentBuilder : ArgumentBuilder
        {
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                var errmes = line.Function.Method.CheckArgumentType(line.Function.Name, args);
                if (errmes != null)
        ...
        private sealed class INT_EXPRESSION_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (line.FunctionCode == FunctionCode.REPEAT)
                {
                    ...
                    var count = GlobalStatic.VariableData.GetSystemVariableToken("COUNT");
                    var repCount = new VariableTerm(count, new IOperandTerm[] {new SingleTerm(0)});
                    repCount.Restructure(exm);
                    return new SpForNextArgment(repCount, new SingleTerm(0), term, new SingleTerm(1));
                }
                var ret = new ExpressionArgument(term);
                if (term is SingleTerm)
                {
                    var i = term.GetIntValue(null);
                    ret.ConstInt = i;
                    ret.IsConst = true;
        ...
        private sealed class INT_ANY_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                var ret = new ExpressionArrayArgument(termList);
                if (terms.Length == 0)
                {
        ...
        private sealed class EXPRESSION_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (terms.Length == 0)
                {
                    var ret = new ExpressionArgument(null);
                    ret.ConstStr = "";
                    ret.ConstInt = 0;
                    ret.IsConst = true;
                    return ret;
        ...
        private sealed class SP_SAVEDATA_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                argumentTypeArray = new[] {typeof(long), typeof(string)};
            }
        ...
        private sealed class SP_TINPUT_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                argumentTypeArray = new[] {typeof(long), typeof(long), typeof(long), typeof(string)};
                minArg = 2;
            }
        ...
        private sealed class SP_TINPUTS_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                argumentTypeArray = new[] {typeof(long), typeof(string), typeof(long), typeof(string)};
                minArg = 2;
            }
        ...
        private sealed class SP_FOR_NEXT_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                argumentTypeArray = new[] {typeof(long), null, typeof(long), typeof(long)};
                minArg = 3;
            }
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (!checkArgumentType(line, exm, terms))
                    return null;
                var varTerm = getChangeableVariable(terms, 1, line);
                if (varTerm == null)
                    return null;
                if (varTerm.Identifier.IsCharacterData)
                {
                    warn("第1引数にキャラクタ変数を指定することはできません", line, 2, false);
                    return null;
                }
        ...
        private sealed class SP_SWAPVAR_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (!checkArgumentType(line, exm, terms))
                    return null;
                var x = getChangeableVariable(terms, 1, line);
                if (x == null)
                    return null;
                var y = getChangeableVariable(terms, 2, line);
                if (y == null)
                    return null;
        ...
        private sealed class VAR_STR_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (terms.Length == 0)
                {
                    var varToken = GlobalStatic.VariableData.GetSystemVariableToken("RESULTS");
                    var varTerm = new VariableTerm(varToken, new IOperandTerm[] {new SingleTerm(0)});
        ...
        private sealed class BIT_ARG_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                //最初の項はいらない
                termList.RemoveAt(0);
                var ret = new BitArgument(varTerm, termList.ToArray());
                for (var i = 0; i < termList.Count; i++)
                    if (termList[i] is SingleTerm)
                    {
                        var bit = ((SingleTerm) termList[i]).Int;
                        if (bit < 0 || bit > 63)
                        {
                            warn(
                                "第" + Strings.StrConv((i + 2).ToString(), VbStrConv.Wide, Config.Language) + "引数(" +
                                bit + ")が範囲(０～６３)を超えています", line, 2, false);
                            return null;
                        }
                    }
                return ret;
        ...
        private sealed class SP_VAR_SET_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                argumentTypeArray = new[] {typeof(void), typeof(void), typeof(long), typeof(long)};
                minArg = 1;
            }
        ...
        private sealed class SP_CVAR_SET_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                argumentTypeArray = new[] {typeof(void), typeof(void), typeof(void), typeof(long), typeof(long)};
                minArg = 1;
            }
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (!varTerm.Identifier.IsCharacterData)
                {
                    warn("第１引数にキャラクタ変数以外の変数を指定することはできません", line, 2, false);
                    return null;
                }
                //1803beta004 暫定CDFLAGを弾く
                if (varTerm.Identifier.IsArray2D)
                {
                    warn("第１引数に二次元配列の変数を指定することはできません", line, 2, false);
                    return null;
                }
                ...
                if (index is SingleTerm && index.GetOperandType() == typeof(string) && varTerm.Identifier.IsArray1D)
                    ...
                    {
                        warn("文字列" + index.GetStrValue(null) + "は変数" + varTerm.Identifier.Name + "の要素ではありません", line, 2,
                            false);
                        return null;
                    }
                if (terms.Length > 3 && !varTerm.Identifier.IsArray1D)
        ...
        private sealed class SP_COLOR_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (terms.Length == 2)
                {
                    warn("SETCOLORの引数の数が不正です(SETCOLORの引数は1個もしくは3個です)", line, 2, false);
                    return null;
                }
                ...
                else
                {
                    ...
                    {
                        arg.ConstInt = (terms[0].GetIntValue(exm) << 16) + (terms[1].GetIntValue(exm) << 8) +
                                       terms[2].GetIntValue(exm);
                        arg.IsConst = true;
                    }
                }
                return arg;
        ...
        private sealed class SP_SPLIT_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                argumentTypeArray = new[] {typeof(string), typeof(string), typeof(string), typeof(long)};
                minArg = 3;
            }
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (!checkArgumentType(line, exm, terms))
                    return null;
                var x = getChangeableVariable(terms, 3, line);
                if (x == null)
                    return null;
                if (!x.Identifier.IsArray1D && !x.Identifier.IsArray2D && !x.Identifier.IsArray3D)
                {
                    warn("第３引数は配列変数でなければなりません", line, 2, false);
                    return null;
                }
                var term = terms.Length >= 4
                    ? getChangeableVariable(terms, 4, line)
                    : new VariableTerm(GlobalStatic.VariableData.GetSystemVariableToken("RESULT"),
                        new IOperandTerm[] {new SingleTerm(0)});
        ...
        private sealed class SP_HTMLSPLIT_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                argumentTypeArray = new[] {typeof(string), typeof(string), typeof(long)};
                minArg = 1;
            }
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                {
                    warn("第２引数は非キャラ型の1次元配列変数でなければなりません", line, 2, false);
                    return null;
                }
                if (terms.Length >= 3)
                ...
                if (term == null)
                {
                    var varToken = GlobalStatic.VariableData.GetSystemVariableToken("RESULT");
        ...
        private sealed class SP_CONTROL_ARRAY_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                argumentTypeArray = new[] {typeof(void), typeof(long), typeof(long)};
            }
        ...
        private sealed class SP_SHIFT_ARRAY_ArgumentBuilder : ArgumentBuilder
        {
            ...
            {
                argumentTypeArray = new[] {typeof(void), typeof(long), typeof(void), typeof(long), typeof(long)};
                minArg = 3;
            }
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (line.FunctionCode == FunctionCode.ARRAYSHIFT)
                    ...
                    {
                        warn("第１引数と第３引数の型が違います", line, 2, false);
                        return null;
                    }
                var term4 = terms.Length >= 4 ? terms[3] : new SingleTerm(0);
                var term5 = terms.Length >= 5 ? terms[4] : null;
        ...
        private sealed class SP_SAVEVAR_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (!checkArgumentType(line, exm, terms))
                    return null;
                var varTokens = new List<VariableToken>();
                for (var i = 2; i < terms.Length; i++)
                {
                    if (terms[i] == null)
                    {
                        warn("第" + (i + 1) + "引数を省略できません", line, 2, false);
                        return null;
                    }
                    var vTerm = getChangeableVariable(terms, i + 1, line);
                    if (vTerm == null)
                        return null;
                    var vToken = vTerm.Identifier;
                    if (vToken.IsCharacterData)
                    {
                        warn("キャラクタ変数" + vToken.Name + "はセーブできません(キャラクタ変数のSAVEにはSAVECHARAを使用します)", line, 2, false);
                        return null;
                    }
                    if (vToken.IsPrivate)
                    {
                        warn("プライベート変数" + vToken.Name + "はセーブできません", line, 2, false);
                        return null;
                    }
                    if (vToken.IsLocal)
                    {
                        warn("ローカル変数" + vToken.Name + "はセーブできません", line, 2, false);
                        return null;
                    }
                    if (vToken.IsConst)
                    {
                        warn("値を変更できない変数はセーブできません", line, 2, false);
                        return null;
                    }
                    if (vToken.IsCalc)
                    {
                        warn("疑似変数はセーブできません", line, 2, false);
                        return null;
                    }
                    if (vToken.IsReference)
                    {
                        warn("参照型変数はセーブできません", line, 2, false);
                        return null;
                    }
                    varTokens.Add(vToken);
                }
                for (var i = 0; i < varTokens.Count; i++)
                for (var j = i + 1; j < varTokens.Count; j++)
                    if (varTokens[i] == varTokens[j])
                    {
                        warn("変数" + varTokens[i].Name + "を二度以上保存しようとしています", line, 1, false);
                        return null;
                    }
                var arg3 = new VariableToken[varTokens.Count];
        ...
        private sealed class SP_SAVECHARA_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                for (var i = 2; i < termList.Count; i++)
                {
                    if (!(termList[i] is SingleTerm))
                        continue;
                    var iValue = termList[i].GetIntValue(null);
                    if (iValue < 0)
                    {
                        warn("キャラ登録番号は正の値でなければなりません", line, 2, false);
                        return null;
                    }
                    if (iValue > int.MaxValue)
                    {
                        warn("キャラ登録番号が32bit符号付整数の上限を超えています", line, 2, false);
                        return null;
                    }
                    for (var j = i + 1; j < termList.Count; j++)
                    {
                        if (!(termList[j] is SingleTerm))
                            continue;
                        if (iValue == termList[j].GetIntValue(null))
                        {
                            warn("キャラ登録番号" + iValue + "を二度以上保存しようとしています", line, 1, false);
                            return null;
                        }
                    }
                }
        ...
        private sealed class SP_REF_ArgumentBuilder : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                var refm = GlobalStatic.IdentifierDictionary.GetRefMethod(id.Code);
                ReferenceToken refVar = null;
                if (refm == null)
                {
                    var token = GlobalStatic.IdentifierDictionary.GetVariableToken(id.Code, null, true);
                    if (token == null || !token.IsReference)
                    {
                        warn("第一引数は関数参照か参照型変数でなければなりません", line, 2, false);
                        return null;
                    }
                ...
                if (refm != null)
                {
                    ...
                    var srcRef = GlobalStatic.IdentifierDictionary.GetRefMethod(srcCode);
                    if (srcRef != null)
                        return new RefArgument(refm, srcRef);
                    var label = GlobalStatic.LabelDictionary.GetNonEventLabel(srcCode);
                    if (label == null)
                    {
                        warn("式中関数" + srcCode + "が見つかりません", line, 2, false);
                        return null;
                    }
                    if (!label.IsMethod)
                    {
                        warn("#FUNCTION(S)属性を持たない関数" + srcCode + "は参照できません", line, 2, false);
                        return null;
                    }
                    var called = CalledFunction.CreateCalledFunctionMethod(label, label.LabelName);
                    return new RefArgument(refm, called);
                }
                if (srcCode == null)
                    return new RefArgument(refVar, name);
                var srcVar = GlobalStatic.IdentifierDictionary.GetVariableToken(srcCode, null, true);
                if (srcVar == null)
                {
                    warn("変数" + srcCode + "が見つかりません", line, 2, false);
                    return null;
                }
                ...
                if (term is SingleTerm)
                {
                    ...
                    if (line.FunctionCode == FunctionCode.ONEINPUT)
                        ...
                        else if (i > 9)
                        {
                            ...
                            i = long.Parse(i.ToString().Remove(1));
                        }
                    ret.ConstInt = i;
                    ret.IsConst = true;
                }
        ...
        private sealed class SP_COPY_ARRAY_Arguments : ArgumentBuilder
        {
            ...
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                if (terms[0] is SingleTerm)
                {
                    if ((vars[0] = GlobalStatic.IdentifierDictionary.GetVariableToken(((SingleTerm) terms[0]).Str, null,
                            true)) == null)
                    {
                    ...
                    {
                        warn("ARRAYCOPY命令の第１引数\"" + ((SingleTerm) terms[0]).Str + "\"はキャラクタ変数です（対応していません）", line, 2,
                            false);
                        return null;
                    }
                }
                ...
                {
                    if ((vars[1] = GlobalStatic.IdentifierDictionary.GetVariableToken(((SingleTerm) terms[1]).Str, null,
                            true)) == null)
                    {
                    ...
                    {
                        warn("ARRAYCOPY命令の第２引数\"" + ((SingleTerm) terms[1]).Str + "\"はキャラクタ変数です（対応していません）", line, 2,
                            false);
                        return null;
                    }
                    if (vars[1].IsConst)
