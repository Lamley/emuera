...
namespace MinorShift.Emuera.GameProc
{
    internal sealed class UserDefinedFunctionArgument
    {
        ...
        public void SetTransporter(ExpressionMediator exm)
        {
            for (var i = 0; i < Arguments.Length; i++)
            {
                ...
                if (isRef[i])
                {
                    var vTerm = (VariableTerm) Arguments[i];
                    if (vTerm.Identifier.IsCharacterData)
                    {
                        var charaNo = vTerm.GetElementInt(0, exm);
                        if (charaNo < 0 || charaNo >= GlobalStatic.VariableData.CharacterList.Count)
                            throw new CodeEE("キャラクタ配列変数" + vTerm.Identifier.Name + "の第１引数(" + charaNo +
                                             ")はキャラ登録番号の範囲外です");
    ...
    internal sealed class CalledFunction
    {
        ...
        public LogicalLine ReturnAddress { get; private set; }
        ...
        {
            var called = new CalledFunction(FunctionName);
