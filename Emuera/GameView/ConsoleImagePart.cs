    ...
    internal class ConsoleImagePart : AConsoleDisplayPart
    {
        ...
        public override void DrawTo(Graphics graph, int pointY, bool isSelecting, bool isBackLog, TextDrawingMode mode)
        {
            ...
            if (img != null)
            {
                if (ia == null)
                    ...
                    graph.DrawImage(img.BaseImage.Bitmap, rect, img.Rectangle.X, img.Rectangle.Y, img.Rectangle.Width,
                        img.Rectangle.Height, GraphicsUnit.Pixel, ia);
            }
            else
            {
        ...
        public override void GDIDrawTo(int pointY, bool isSelecting, bool isBackLog)
        {
            ...
            if (isSelecting && cImageB != null)
                img = cImageB;
            if (img != null)
                GDI.DrawImage(PointX + destRect.X, pointY + destRect.Y, Width, destRect.Height, img.BaseImage.GDIhDC,
                    img.Rectangle);
            else
