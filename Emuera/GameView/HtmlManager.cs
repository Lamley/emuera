...
namespace MinorShift.Emuera.GameView
{
    ...
    internal static class HtmlManager
    {
        ...
        private static readonly Dictionary<char, string> repDic = new Dictionary<char, string>();
        ...
        public static string DisplayLine2Html(ConsoleDisplayLine[] lines, bool needPandN)
        {
            ...
            for (var dispCounter = 0; dispCounter < lines.Length; dispCounter++)
            {
                if (dispCounter != 0)
                    b.Append("<br>");
                var buttons = lines[dispCounter].Buttons;
                for (var buttonCounter = 0; buttonCounter < buttons.Length; buttonCounter++)
                {
                    ...
                    var hasTag = buttons[buttonCounter].IsButton || titleValue != null
                                 || buttons[buttonCounter].PointXisLocked;
                    if (hasTag)
                    {
                        if (buttons[buttonCounter].IsButton)
                        {
                            var attrValue = Escape(buttons[buttonCounter].Inputs);
                    ...
                    var parts = buttons[buttonCounter].StrArray;
                    for (var cssCounter = 0; cssCounter < parts.Length; cssCounter++)
                        if (parts[cssCounter] is ConsoleStyledString)
                        {
                            var css = parts[cssCounter] as ConsoleStyledString;
        ...
        public static string[] HtmlTagSplit(string str)
        {
            ...
            var ret = new string[strList.Count];
            strList.CopyTo(ret);
            return ret;
        }
        ...
        {
            var cssList = new List<AConsoleDisplayPart>();
            var buttonList = new List<ConsoleButtonString>();
            var st = new StringStream(str);
            int found;
            var hasComment = str.IndexOf("<!--") >= 0;
            var hasReturn = str.IndexOf('\n') >= 0;
            ...
            while (!st.EOS)
            {
                ...
                if (found > 0)
                {
                    var txt = Unescape(st.Substring(st.CurrentPosition, found));
                    cssList.Add(new ConsoleStyledString(txt, state.GetSS()));
        ...
        public static string Unescape(string str)
        {
            ...
            while (index < str.Length)
            {
                ...
                var escWordRow = str.Substring(index + 1, found - index - 1);
                index = found + 1;
                var escWord = escWordRow.ToLower();
                var unicode = 0;
                switch (escWord)
                {
                    case "nbsp":
        ...
        public static string GetColorToString(Color color)
        {
            var b = new StringBuilder();
            b.Append("#");
            var colorValue = color.R * 0x10000 + color.G * 0x100 + color.B;
        ...
        private static AConsoleDisplayPart tagAnalyze(HtmlAnalzeState state, StringStream st)
        {
            ...
            if (endTag)
            {
                ...
                {
                    case "b":
                        endStyle = FontStyle.Bold;
                        goto case "s";
                    case "i":
                        endStyle = FontStyle.Italic;
                        goto case "s";
                    case "u":
                        endStyle = FontStyle.Underline;
                        goto case "s";
                    case "s":
            ...
            var tempUseMacro = LexicalAnalyzer.UseMacro;
            ...
            try
            {
                ...
                if (st.Current != '>')
                    wc = LexicalAnalyzer.Analyse(st, LexEndWith.GreaterThan,
                        LexAnalyzeFlag.AllowAssignment | LexAnalyzeFlag.AllowSingleQuotationStr);
            }
            finally
            {
            ...
            if (string.IsNullOrEmpty(tag))
                goto error;
            IdentifierWord word;
            var newStyle = FontStyle.Strikeout;
            switch (tag.ToLower())
            {
                case "b":
                ...
                case "p":
                {
                    ...
                    wc.ShiftNext();
                    var op = wc.Current as OperatorWord;
                    wc.ShiftNext();
                    var attr = wc.Current as LiteralStringWord;
                    wc.ShiftNext();
                    ...
                    var attrValue = Unescape(attr.Str);
                ...
                case "nonbutton":
                {
                    ...
                    var buttonTag = new HtmlAnalzeStateButtonTag();
                    var isButton = tag.ToLower() == "button";
                    string attrValue = null;
                    string value = null;
                    //if (wc == null)
                    //	throw new CodeEE("<" + tag + ">タグに属性が設定されていません");
                ...
                case "font":
                {
                    if (wc == null)
                        throw new CodeEE("<" + tag + ">タグに属性が設定されていません");
                    var font = new HtmlAnalzeStateFontTag();
                    while (!wc.EOL)
                    {
                        word = wc.Current as IdentifierWord;
                        wc.ShiftNext();
                    ...
                    if (state.FonttagList.Count > 0)
                    {
                        var oldFont = state.FonttagList[state.FonttagList.Count - 1];
                        if (font.Color < 0)
        ...
        private static int stringToColorInt32(string str)
        {
            ...
            if (str[0] == '#')
            {
                var colorvalue = str.Substring(1);
                try
                {
        ...
        private sealed class HtmlAnalzeState
        {
            ...
            public readonly List<HtmlAnalzeStateFontTag> FonttagList = new List<HtmlAnalzeStateFontTag>();
            ...
            public StringStyle GetSS()
            {
                ...
                {
                    var font = FonttagList[FonttagList.Count - 1];
