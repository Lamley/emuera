...
namespace MinorShift.Emuera
{
    ...
    {
        private static readonly char[] badSymbolAsIdentifier =
        {
            '+', '-', '*', '/', '%', '=', '!', '<', '>', '|', '&', '^', '~',
            ' ', '　', '\t',
            '\"', '(', ')', '{', '}', '[', ']', ',', '.', ':',
        ...
        private readonly List<string> disableList = new List<string>();
        private readonly Dictionary<string, FunctionIdentifier> instructionDic;
        private readonly Dictionary<string, VariableLocal> localvarTokenDic;
        private readonly Dictionary<string, FunctionMethod> methodDic;


        private readonly Dictionary<string, DefinedNameType> nameDic = new Dictionary<string, DefinedNameType>();

        private readonly List<string> privateDimList = new List<string>();

        private readonly Dictionary<string, UserDefinedRefMethod> refmethodDic;
        //Dictionary<string, VariableToken> userDefinedVarDic = new Dictionary<string, VariableToken>();

        private VariableData varData;
        private readonly Dictionary<string, VariableToken> varTokenDic;
        ...
        public IdentifierDictionary(VariableData varData)
        {
            ...
            foreach (var pair in localvarTokenDic)
                nameDic.Add(pair.Key, DefinedNameType.SystemVariable);

            foreach (var pair in instructionDic)
                //Methodと被る
                //1808a3 SAVEDATAも
                if (!nameDic.ContainsKey(pair.Key))
        ...
        public void CheckUserLabelName(ref string errMes, ref int warnLevel, bool isFunction, string labelName)
        {
            ...
            if (char.IsDigit(labelName[0]) &&
                labelName[0].ToString().Length == LangManager.GetStrlenLang(labelName[0].ToString()))
            {
        ...
        //1807 ErbLoaderに移動
        private readonly Dictionary<string, DefineMacro> macroDic = new Dictionary<string, DefineMacro>();
        ...
        public VariableToken GetVariableToken(string key, string subKey, bool allowPrivate)
        {
            ...
            if (localvarTokenDic.ContainsKey(key))
            {
                ...
                {
                    ParserMediator.Warn("コード中でローカル変数を@付きで呼ぶことは推奨されません(代わりに*.ERHファイルの利用を検討してください)", line, 1, false,
                        false);
                    if (Config.ICFunction)
                        subKey = subKey.ToUpper();
                }
                var retLocal = localvarTokenDic[key].GetExistLocalVariableToken(subKey);
                if (retLocal == null)
            ...
            bool userDefinedOnly)
        {
            ...
            if (labelDic != null && labelDic.Initialized)
            {
                ...
                var func = labelDic.GetNonEventLabel(codeStr);
                if (func != null)
                {
                    ...
                    if (!methodDic.ContainsKey(codeStr))
                        throw new CodeEE("#FUNCTIONが定義されていない関数(" + func.Position.Filename + ":" + func.Position.LineNo +
                                         "行目)を式中で呼び出そうとしました");
                }
            }
            if (userDefinedOnly)
            ...
            if (!methodDic.TryGetValue(codeStr, out method))
                return null;
            var errmes = method.CheckArgumentType(codeStr, arguments);
            if (errmes != null)
                throw new CodeEE(errmes);
