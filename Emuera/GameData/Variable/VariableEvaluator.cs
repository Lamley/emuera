...
namespace MinorShift.Emuera.GameData.Variable
{
    internal sealed class VariableEvaluator : IDisposable
    {
        ...
        public VariableEvaluator(GameBase gamebase, ConstantData constant)
        {
            ...
            Constant = constant;
            VariableData = new VariableData(gamebase, constant);
            GlobalStatic.VariableData = VariableData;
        }
        ...
        {
            if (MASTER < 0 || MASTER >= VariableData.CharacterList.Count)
                return;
            VariableData.CharacterList[(int) MASTER]
                .DataString[(int) (VariableCode.NAME & VariableCode.__LOWERCASE__)] = "イカサマ";
            VariableData.CharacterList[(int) MASTER]
                .DataString[(int) (VariableCode.CALLNAME & VariableCode.__LOWERCASE__)] = "イカサマ";
            VariableData.CharacterList[(int) MASTER]
                .DataString[(int) (VariableCode.NICKNAME & VariableCode.__LOWERCASE__)] = "イカサマ";
        }
        ...
        public void SetResultX(List<long> values)
        {
            ...
            {
                if (i >= VariableData.DataIntegerArray[(int) (VariableCode.RESULT & VariableCode.__LOWERCASE__)].Length)
                    return;
                VariableData.DataIntegerArray[(int) (VariableCode.RESULT & VariableCode.__LOWERCASE__)][i] = values[i];
            }
        }
        ...
        {
            for (var i = 0; i < (int) maxlv; i++)
                if (pl < VariableData.DataIntegerArray[(int) (VariableCode.PALAMLV & VariableCode.__LOWERCASE__)][i + 1]
                )
                    return i;
            return maxlv;
        }
        ...
        {
            for (var i = 0; i < (int) maxlv; i++)
                if (pl < VariableData.DataIntegerArray[(int) (VariableCode.EXPLV & VariableCode.__LOWERCASE__)][i + 1])
                    return i;
            return maxlv;
        }
        ...
        public void SetValueAllEachChara(FixedVariableTerm p, SingleTerm index, long srcValue, int start, int end)
        {
            ...
            //一応チェック済み
            //throw new ExeEE("READONLYでないCALC変数の代入処理が設定されていない");
            if (VariableData.CharacterList.Count == 0)
                return;

            var chara = VariableData.CharacterList[0];
            long indexNum = -1;

            if (p.Identifier.IsArray1D)
            {
                if (index.GetOperandType() == typeof(long))
                    indexNum = index.Int;
                else
                ...
                if (indexNum < 0 || indexNum >= ((long[]) p.Identifier.GetArrayChara(0)).Length)
                    throw new CodeEE("キャラクタ配列変数" + p.Identifier.Name + "の第２引数(" + indexNum + ")は配列の範囲外です");
            }
        ...
        public long GetArraySum(FixedVariableTerm p, long index1, long index2)
        {
            ...
            {
                if (p.Identifier.IsArray1D)
                    for (var i = (int) index1; i < (int) index2; i++)
        ...
        public long GetMatch(FixedVariableTerm p, long target, long start, long end)
        {
            ...
            for (var i = (int) start; i < (int) end; i++)
                if (p.Identifier.GetIntValue(GlobalStatic.EMediator,
                        p.Identifier.IsCharacterData ? new[] {p.Index1, i} : new long[] {i}) == target)
                    ret++;
        ...
        {
            long ret = 0;
            var targetIsNullOrEmpty = string.IsNullOrEmpty(target);

            for (var i = (int) start; i < (int) end; i++)
                if (p.Identifier.GetStrValue(GlobalStatic.EMediator,
                        p.Identifier.IsCharacterData ? new[] {p.Index1, i} : new long[] {i}) == target ||
                    targetIsNullOrEmpty && string.IsNullOrEmpty(p.Identifier.GetStrValue(GlobalStatic.EMediator,
                        p.Identifier.IsCharacterData ? new[] {p.Index1, i} : new long[] {i})))
                    ret++;
        ...
        public long GetMatchChara(FixedVariableTerm p, string target, long start, long end)
        {
            ...
            for (var i = (int) start; i < (int) end; i++)
                if (p.Identifier.GetStrValue(GlobalStatic.EMediator, new[] {i, p.Index2, p.Index3}) == target ||
                    targetIsNullOrEmpty &&
                    string.IsNullOrEmpty(
                        p.Identifier.GetStrValue(GlobalStatic.EMediator, new[] {i, p.Index2, p.Index3})))
                    ret++;
        ...
        public long FindElement(FixedVariableTerm p, long target, long start, long end, bool isExact, bool isLast)
        {
            ...
            if (isLast)
                for (var i = (int) end - 1; i >= (int) start; i--)
                    if (target == array[i])
                        return i;
            else
                for (var i = (int) start; i < (int) end; i++)
            ...
            if (isLast)
                for (var i = (int) end - 1; i >= (int) start; i--)
                    if (isExact)
                    {
                        ...
                        if (array[i] == null)
                            continue;
                        var match = target.Match(array[i]);
        ...
        {
            long value;
            var ret = p.Identifier.GetIntValue(GlobalStatic.EMediator,
                p.Identifier.IsCharacterData ? new[] {p.Index1, start} : new[] {start});
            for (var i = (int) start + 1; i < (int) end; i++)
            {
                value = p.Identifier.GetIntValue(GlobalStatic.EMediator,
                    p.Identifier.IsCharacterData ? new[] {p.Index1, i} : new long[] {i});
                if (isMax)
                {
                    if (value > ret)
        ...
        public void ShiftArray(FixedVariableTerm p, int shift, long def, int start, int num)
        {
            ...
            var length = num - Math.Abs(shift);
            if (shift < 0)
            {
                sourceStart = -shift;
                destStart = start;
            }
            var temp = new long[num];
            ...
            else
            {
                if (length > 0)
                {
                    for (var i = start + length; i < start + num; i++)
                        array[i] = def;
                }
                else
                {
        ...
        public void RemoveArray(FixedVariableTerm p, int start, int num)
        {
            if (p.Identifier.IsInteger)
            {
                ...
                if (num <= 0)
                    num = array.Length;
                var temp = new long[array.Length];
                //array.CopyTo(temp, 0);
            ...
            else
            {
                ...
                if (num <= 0)
                    num = arrays.Length;
                var temps = new string[arrays.Length];
        ...
        public void SortArray(FixedVariableTerm p, SortOrder order, int start, int num)
        {
            ...
            if (p.Identifier.IsInteger)
            {
                ...
                if (order == SortOrder.ASCENDING)
                    ...
                    Array.Sort(temp, delegate(long a, long b) { return b.CompareTo(a); });
        ...
        public void CopyArray(VariableToken var1, VariableToken var2)
        {
            if (var1.IsInteger)
            {
                if (var1.IsArray1D)
                {
                    var array1 = (long[]) var1.GetArray();
                    var array2 = (long[]) var2.GetArray();
                ...
                else if (var1.IsArray2D)
                {
                    var array1 = (long[,]) var1.GetArray();
                    var array2 = (long[,]) var2.GetArray();
                    var length1 = array1.GetLength(0) >= array2.GetLength(0)
                        ? array2.GetLength(0)
                        : array1.GetLength(0);
                    var length2 = array1.GetLength(1) >= array2.GetLength(1)
                        ? array2.GetLength(1)
                        : array1.GetLength(1);
                ...
                else
                {
                    ...
                    var length2 = array1.GetLength(1) >= array2.GetLength(1)
                        ? array2.GetLength(1)
                        : array1.GetLength(1);
                    var length3 = array1.GetLength(2) >= array2.GetLength(2)
                        ? array2.GetLength(2)
                        : array1.GetLength(2);
                    for (var i = 0; i < length1; i++)
                    for (var j = 0; j < length2; j++)
            ...
            else
            {
                if (var1.IsArray1D)
                {
                    var array1 = (string[]) var1.GetArray();
                    var array2 = (string[]) var2.GetArray();
                    var length = array1.Length >= array2.Length ? array2.Length : array1.Length;
                    for (var i = 0; i < length; i++)
                        array2[i] = array1[i];
                }
        ...
        public string GetHavingItemsString()
        {
            ...
            var length = Math.Min(array.Length, itemnames.Length);
            var count = 0;
        ...
        {
            var builder = new StringBuilder(100);
            if (target < 0 || target >= VariableData.CharacterList.Count)
                throw new CodeEE("存在しない登録キャラクタを参照しようとしました");
            var chara = VariableData.CharacterList[(int) target];
            long[] array = null;
        ...
        public string GetCharacterParamString(long target, int paramCode)
        {
            ...
            var chara = VariableData.CharacterList[(int) target];
            var param = chara.DataIntegerArray[(int) (VariableCode.PALAM & VariableCode.__LOWERCASE__)][paramCode];
            var paramlv = VariableData.DataIntegerArray[(int) (VariableCode.PALAMLV & VariableCode.__LOWERCASE__)];
        ...
        {
            var tmpl = Constant.GetCharacterTemplate(charaTmplNo);
            if (tmpl == null)
                throw new CodeEE("定義していないキャラクタを作成しようとしました");
            var chara = new CharacterData(Constant, tmpl, VariableData);
            VariableData.CharacterList.Add(chara);
        }

        public void AddCharacter_UseSp(long charaTmplNo, bool isSp)
        {
            var tmpl = Constant.GetCharacterTemplate_UseSp(charaTmplNo, isSp);
            if (tmpl == null)
                throw new CodeEE("定義していないキャラクタを作成しようとしました");
            var chara = new CharacterData(Constant, tmpl, VariableData);
            VariableData.CharacterList.Add(chara);
        ...
        {
            var tmpl = Constant.GetCharacterTemplateFromCsvNo(CsvNo);
            if (tmpl == null)
                //throw new CodeEE("定義していないキャラクタを作成しようとしました");
        ...
        {
            var tmpl = Constant.GetPseudoChara();
            var chara = new CharacterData(Constant, tmpl, VariableData);
            VariableData.CharacterList.Add(chara);
        }
        ...
        {
            if (charaNo < 0 || charaNo >= VariableData.CharacterList.Count)
                throw new CodeEE("存在しない登録キャラクタ(" + charaNo + ")を削除しようとしました");
            VariableData.CharacterList[(int) charaNo].Dispose();
            VariableData.CharacterList.RemoveAt((int) charaNo);
        }

        public void DelCharacter(long[] charaNoList)
        {
            var DelList = new List<CharacterData>();
            foreach (var charaNo in charaNoList)
            {
                if (charaNo < 0 || charaNo >= VariableData.CharacterList.Count)
                    throw new CodeEE("存在しない登録キャラクタ(" + charaNoList + ")を削除しようとしました");
                var chara = VariableData.CharacterList[(int) charaNo];
                if (DelList.Contains(chara))
                    throw new CodeEE("同一の登録キャラクタ番号(" + charaNo + ")が複数回指定されました");
                DelList.Add(chara);
                chara.Dispose();
            }
            foreach (var chara in DelList)
                VariableData.CharacterList.Remove(chara);
        }
        ...
        public void DelAllCharacter()
        {
            if (VariableData.CharacterList.Count == 0)
                return;
            foreach (var chara in VariableData.CharacterList)
                chara.Dispose();
            VariableData.CharacterList.Clear();
        }
        ...
        public void PickUpChara(long[] NoList)
        {
            ...
            if (pickList.Count < VariableData.CharacterList.Count)
                for (var i = VariableData.CharacterList.Count - 1; i >= pickList.Count; i--)
                    DelCharacter(i);
        }
        ...
        public void ResetData()
        {
            ...
            VariableData.SetDefaultLocalValue();
            VariableData.SetDefaultValue(Constant);
            foreach (var chara in VariableData.CharacterList)
                chara.Dispose();
            VariableData.CharacterList.Clear();
        }
        ...
        {
            VariableData.SetDefaultGlobalValue();
        }

        public void CopyChara(long x, long y)
        {
            if (x < 0 || x >= VariableData.CharacterList.Count)
                throw new CodeEE("コピー元のキャラクタが存在しません");
            if (y < 0 || y >= VariableData.CharacterList.Count)
                throw new CodeEE("コピー先のキャラクタが存在しません");
            VariableData.CharacterList[(int) x].CopyTo(VariableData.CharacterList[(int) y]);
        }
        ...
        public void AddCopyChara(long x)
        {
            if (x < 0 || x >= VariableData.CharacterList.Count)
                throw new CodeEE("コピー元のキャラクタが存在しません");
            AddPseudoCharacter();
            VariableData.CharacterList[(int) x]
                .CopyTo(VariableData.CharacterList[VariableData.CharacterList.Count - 1]);
        }

        public void SwapChara(long x, long y)
        {
            if (x < 0 || x >= VariableData.CharacterList.Count || y < 0 || y >= VariableData.CharacterList.Count)
                throw new CodeEE("存在しない登録キャラクタを入れ替えようとしました");
            if (x == y)
                return;
            var data = VariableData.CharacterList[(int) y];
            VariableData.CharacterList[(int) y] = VariableData.CharacterList[(int) x];
            VariableData.CharacterList[(int) x] = data;
        }

        public void SortChara(VariableToken sortkey, long elem, SortOrder sortorder, bool fixMaster)
        {
            if (VariableData.CharacterList.Count <= 1)
                return;
            ...
            if (MASTER >= 0 && MASTER < VariableData.CharacterList.Count)
                masterChara = VariableData.CharacterList[(int) MASTER];
            if (TARGET >= 0 && TARGET < VariableData.CharacterList.Count)
                targetChara = VariableData.CharacterList[(int) TARGET];
            if (ASSI >= 0 && ASSI < VariableData.CharacterList.Count)
                assiChara = VariableData.CharacterList[(int) ASSI];

            for (var i = 0; i < VariableData.CharacterList.Count; i++)
            {
                VariableData.CharacterList[i].temp_CurrentOrder = i;
                VariableData.CharacterList[i].SetSortKey(sortkey, elem);
            }
            if (fixMaster && masterChara != null)
            {
                if (VariableData.CharacterList.Count <= 2)
                    return;
                VariableData.CharacterList.Remove(masterChara);
            }
            if (sortorder == SortOrder.ASCENDING)
                VariableData.CharacterList.Sort(CharacterData.AscCharacterComparison);
            else // if (sortorder == SortOrder.DESENDING)
                VariableData.CharacterList.Sort(CharacterData.DescCharacterComparison);
            //引数解析でチェック済み
            //else
            //    throw new ExeEE("ソート順序不明");

            if (fixMaster && masterChara != null)
                VariableData.CharacterList.Insert((int) MASTER, masterChara);
            for (var i = 0; i < VariableData.CharacterList.Count; i++)
                VariableData.CharacterList[i].temp_CurrentOrder = i;
        ...
        internal long FindChara(VariableToken varID, long elem64, string word, long startIndex, long lastIndex,
            bool isLast)
        {
            if (startIndex >= lastIndex)
                return -1;
            var fvp = new FixedVariableTerm(varID);
            if (varID.IsArray1D)
            {
                fvp.Index2 = elem64;
            }
            ...
            var count = VariableData.CharacterList.Count;
            if (isLast)
                for (var i = lastIndex - 1; i >= startIndex; i--)
                {
                    fvp.Index1 = i;
        ...
        internal long FindChara(VariableToken varID, long elem64, long word, long startIndex, long lastIndex,
            bool isLast)
        {
            if (startIndex >= lastIndex)
                return -1;
        ...
        {
            int i;
            for (i = 0; i < VariableData.CharacterList.Count; i++)
                if (VariableData.CharacterList[i].NO == charaNo)
                    return i;
            return -1;
        }
        ...
        public long GetChara_UseSp(long charaNo, bool getSp)
        {
            ...
            for (i = 0; i < VariableData.CharacterList.Count; i++)
                if (VariableData.CharacterList[i].NO == charaNo)
                {
                    var isSp = VariableData.CharacterList[i].CFlag[0] != 0;
                    if (isSp == getSp)
                        return i;
                }
            return -1;
        ...
        public long ExistCsv(long charaNo, bool getSp)
        {
            ...
            var tmpl = Constant.GetCharacterTemplate_UseSp(charaNo, getSp);
            if (tmpl == null)
                return 0;
            return 1;
        }
        ...
        public long GetCharacterIntfromCSVData(long charaTmplNo, CharacterIntData type, bool isSp, long arg2Long)
        {
            ...
            var arg2 = (int) arg2Long;
            Dictionary<int, long> intDic = null;
            switch (type)
            {
        ...
        public void UpdateInBeginTrain()
        {
            ...
            long[] array;
            string[] sarray;
            array = VariableData.DataIntegerArray[(int) (VariableCode.TFLAG & VariableCode.__LOWERCASE__)];
            for (var i = 0; i < array.Length; i++)
                array[i] = 0;
            sarray = VariableData.DataStringArray[(int) (VariableCode.TSTR & VariableCode.__LOWERCASE__)];
        ...
        public void UpdateAfterShowUsercom()
        {
            ...
            long[] array;
            array = VariableData.DataIntegerArray[(int) (VariableCode.UP & VariableCode.__LOWERCASE__)];
            for (var i = 0; i < array.Length; i++)
                array[i] = 0;
            array = VariableData.DataIntegerArray[(int) (VariableCode.DOWN & VariableCode.__LOWERCASE__)];
            for (var i = 0; i < array.Length; i++)
                array[i] = 0;
            array = VariableData.DataIntegerArray[(int) (VariableCode.LOSEBASE & VariableCode.__LOWERCASE__)];
            for (var i = 0; i < array.Length; i++)
                array[i] = 0;
            foreach (var chara in VariableData.CharacterList)
            {
        ...
        {
            long[] up, down, param;
            var paramname = Constant.GetCsvNameList(VariableCode.PALAMNAME);
            up = VariableData.DataIntegerArray[(int) (VariableCode.UP & VariableCode.__LOWERCASE__)];
            down = VariableData.DataIntegerArray[(int) (VariableCode.DOWN & VariableCode.__LOWERCASE__)];
            var target = TARGET;
            if (target < 0 || target >= VariableData.CharacterList.Count)
                goto end;
            var chara = VariableData.CharacterList[(int) target];
        ...
        private void setDefaultStain(CharacterData chara)
        {
            ...
            if (array.Length >= Config.StainDefault.Count)
            {
                ...
                for (var i = Config.StainDefault.Count; i < array.Length; i++)
                    array[i] = 0;
            }
            else
            {
        ...
        {
            if (no < 0 || no >= VariableData.CharacterList.Count)
                throw new CodeEE("存在しないキャラクターを参照しようとしました");
            var chara = VariableData.CharacterList[(int) no];
            setDefaultStain(chara);
        }
        ...
        public bool ItemSales(long itemNo)
        {
            ...
            var itemNames = Constant.GetCsvNameList(VariableCode.ITEMNAME);
            if (itemNo < 0 || itemNo >= itemSales.Length || itemNo >= itemNames.Length)
                return false;
            var index = (int) itemNo;
        ...
        public bool BuyItem(long itemNo)
        {
            if (!ItemSales(itemNo))
                return false;
            var itemPrice = Constant.ItemPrice;
        ...
        {
            var resary = VariableData.DataIntegerArray[(int) (VariableCode.RESULT & VariableCode.__LOWERCASE__)];
            resary[0] = ary.Length;
        ...
        {
            return string.Format("{0}save{1:00}.sav", Config.SavDir, index);
        }

        private string getSaveDataPath(string s)
        {
            return string.Format("{0}save{1:00}.sav", Config.SavDir, s);
        }

        private string getSaveDataPathV(int index)
        {
            return Program.DatDir + string.Format("var_{0:00}.dat", index);
        }

        private string getSaveDataPathC(int index)
        {
            return Program.DatDir + string.Format("chara_{0:00}.dat", index);
        }
        ...
        public List<string> GetDatFiles(bool charadat, string pattern)
        {
            ...
            if (!Directory.Exists(Program.DatDir))
                return files;
            var searchPattern = "var_" + pattern + ".dat";
            if (charadat)
                searchPattern = "chara_" + pattern + ".dat";
            var pathes = Directory.GetFiles(Program.DatDir, searchPattern, SearchOption.TopDirectoryOnly);
        ...
        public EraDataResult CheckDataByFilename(string filename, EraSaveFileType type)
        {
            ...
            try
            {
                ...
                var fileType = bReader.ReadFileType();
                if (type != fileType)
                {
                    result.State = EraDataState.ETC_ERROR;
                    result.DataMes = "セーブデータが壊れています";
        ...
        public void SaveChara(string savename, string savMes, int[] charas)
        {
            ...
            var filepath = getSaveDataPathC(savename);
            ...
            try
            {
                ...
                for (var i = 0; i < charas.Length; i++)
                    VariableData.CharacterList[charas[i]].SaveToStreamBinary(bWriter, VariableData);
                bWriter.WriteEOF();
                //RESULT = 1;
            }
            //catch (Exception)
        ...
        public void LoadChara(string savename)
        {
            ...
            try
            {
                var addCharaList = new List<CharacterData>();
                ...
                if (!gamebase.UniqueCodeEqualTo(bReader.ReadInt64()))
                    return;
                var version = bReader.ReadInt64();
                if (!gamebase.CheckVersion(version))
                    return;
                bReader.ReadString(); //saveMes
                var loadnum = bReader.ReadInt64();
                for (var i = 0; i < loadnum; i++)
                {
                    var chara = new CharacterData(Constant, VariableData);
                    chara.LoadFromStreamBinary(bReader);
                    addCharaList.Add(chara);
                }
                VariableData.CharacterList.AddRange(addCharaList);
                RESULT = 1;
            }
            //catch (Exception)
            //{
        ...
        public void SaveVariable(string savename, string savMes, VariableToken[] vars)
        {
            CreateDatFolder();
            CheckDatFilename(savename);
            var filepath = getSaveDataPathV(savename);
            EraBinaryDataWriter bWriter = null;
            FileStream fs = null;
            try
            {
        ...
        public void SaveToStream(EraDataWriter writer, string saveDataText)
        {
            ...
            writer.Write(VariableData.CharacterList.Count);
            for (var i = 0; i < VariableData.CharacterList.Count; i++)
                VariableData.CharacterList[i].SaveToStream(writer);
            VariableData.SaveToStream(writer);
            writer.EmuStart();
            for (var i = 0; i < VariableData.CharacterList.Count; i++)
                VariableData.CharacterList[i].SaveToStreamExtended(writer);
            VariableData.SaveToStreamExtended(writer);
        }
        ...
        public void LoadFromStream(EraDataReader reader)
        {
            ...
            var text = reader.ReadString(); //PUTFORM
            VariableData.SetDefaultValue(Constant);
            VariableData.SetDefaultLocalValue();
            VariableData.LastLoadVersion = version;
            //varData.LastLoadNo = dataIndex;
            VariableData.LastLoadText = text;

            var charaCount = (int) reader.ReadInt64();
            VariableData.CharacterList.Clear();
            ...
            VariableData.LoadFromStream(reader);
            if (reader.SeekEmuStart())
            {
                if (reader.DataVersion < 1803) //キャラ2次元配列追加以前
                    for (var i = 0; i < charaCount; i++)
                        VariableData.CharacterList[i].LoadFromStreamExtended_Old1802(reader);
                else
                    for (var i = 0; i < charaCount; i++)
                        VariableData.CharacterList[i].LoadFromStreamExtended(reader);
                VariableData.LoadFromStreamExtended(reader, reader.DataVersion);
            }
        }

        public bool SaveGlobal()
        {
            var filepath = getSaveDataPathG();
            ...
            try
            {
                ...
                if (Config.SystemSaveInBinary)
                {
                    ...
                    VariableData.SaveGlobalToStreamBinary(bWriter);
                    bWriter.WriteEOF();
                }
                else
                {
                    ...
                    writer.Write(gamebase.ScriptUniqueCode);
                    writer.Write(gamebase.ScriptVersion);
                    VariableData.SaveGlobalToStream(writer);
                    writer.EmuStart();
                    VariableData.SaveGlobalToStream1808(writer);
                }
            }
        ...
        public bool LoadGlobal()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!gamebase.CheckVersion(version))
                        return false;
                    VariableData.LoadGlobalFromStream(reader);
                    if (reader.SeekEmuStart())
                        VariableData.LoadGlobalFromStream1808(reader);
                }
                return true;
            }
            catch
        ...
        public void SaveToStreamBinary(EraBinaryDataWriter bWriter, string saveDataText)
        {
            ...
            bWriter.WriteInt64(VariableData.CharacterList.Count);
            for (var i = 0; i < VariableData.CharacterList.Count; i++)
                VariableData.CharacterList[i].SaveToStreamBinary(bWriter, VariableData);
            VariableData.SaveToStreamBinary(bWriter);
            bWriter.WriteEOF();
        }
        ...
        public void LoadFromStreamBinary(EraBinaryDataReader bReader)
        {
            ...
            if (!gamebase.CheckVersion(version))
                throw new FileEE("セーブデータのバーションが異なります");
            var text = bReader.ReadString(); //PUTFORM
            VariableData.SetDefaultValue(Constant);
            VariableData.SetDefaultLocalValue();
            VariableData.LastLoadVersion = version;
            //varData.LastLoadNo = dataIndex;
            ...
            var charaCount = (int) bReader.ReadInt64();
            VariableData.CharacterList.Clear();
            for (var i = 0; i < charaCount; i++)
            {
                var chara = new CharacterData(Constant, VariableData);
        ...
        {
            var filepath = getSaveDataPath(dataIndex);
            if (!File.Exists(filepath))
            ...
            try
            {
                ...
                VariableData.LastLoadNo = dataIndex;
            }
            finally
            {
                if (reader != null)
        ...
        public long[] RESULT_ARRAY =>
            VariableData.DataIntegerArray[(int) (VariableCode.RESULT & VariableCode.__LOWERCASE__)];

        public long RESULT
        {
            get => VariableData.DataIntegerArray[(int) (VariableCode.RESULT & VariableCode.__LOWERCASE__)][0];
            set => VariableData.DataIntegerArray[(int) (VariableCode.RESULT & VariableCode.__LOWERCASE__)][0] = value;
        }

        public long COUNT
        {
            get => VariableData.DataIntegerArray[(int) (VariableCode.COUNT & VariableCode.__LOWERCASE__)][0];
            set => VariableData.DataIntegerArray[(int) (VariableCode.COUNT & VariableCode.__LOWERCASE__)][0] = value;
        }
        ...
        public string RESULTS
        {
            get
            {
                var ret = VariableData.DataStringArray[(int) (VariableCode.RESULTS & VariableCode.__LOWERCASE__)][0];
                if (ret == null)
                    return "";
                return ret;
            }
            set => VariableData.DataStringArray[(int) (VariableCode.RESULTS & VariableCode.__LOWERCASE__)][0] = value;
        }

        public string[] RESULTS_ARRAY =>
            VariableData.DataStringArray[(int) (VariableCode.RESULTS & VariableCode.__LOWERCASE__)];

        public long TARGET
        {
            get => VariableData.DataIntegerArray[(int) (VariableCode.TARGET & VariableCode.__LOWERCASE__)][0];
            set => VariableData.DataIntegerArray[(int) (VariableCode.TARGET & VariableCode.__LOWERCASE__)][0] = value;
        }

        public long[] SELECTCOM_ARRAY =>
            VariableData.DataIntegerArray[(int) (VariableCode.SELECTCOM & VariableCode.__LOWERCASE__)];

        public long SELECTCOM
        {
            get => VariableData.DataIntegerArray[(int) (VariableCode.SELECTCOM & VariableCode.__LOWERCASE__)][0];
            set => VariableData.DataIntegerArray[(int) (VariableCode.SELECTCOM & VariableCode.__LOWERCASE__)][0] =
                value;
        }
        ...
        public long[] ITEMSALES =>
            VariableData.DataIntegerArray[(int) (VariableCode.ITEMSALES & VariableCode.__LOWERCASE__)];

        public long[] ITEMPRICE => Constant.ItemPrice;

        private long[] ITEM => VariableData.DataIntegerArray[(int) (VariableCode.ITEM & VariableCode.__LOWERCASE__)];

        public long[] RANDDATA =>
            VariableData.DataIntegerArray[(int) (VariableCode.RANDDATA & VariableCode.__LOWERCASE__)];

        public string SAVEDATA_TEXT
        {
            get => VariableData.DataString[(int) (VariableCode.SAVEDATA_TEXT & VariableCode.__LOWERCASE__)];
            set => VariableData.DataString[(int) (VariableCode.SAVEDATA_TEXT & VariableCode.__LOWERCASE__)] = value;
        }

        public long CHARANUM => VariableData.CharacterList.Count;


        private long get_Variable_canforbid(VariableCode code)
        {
            var array = VariableData.DataIntegerArray[(int) (code & VariableCode.__LOWERCASE__)];
            if (array.Length == 0)
                return -1;
            return array[0];
        }
