...
namespace MinorShift.Emuera.GameData.Expression
{
    ...
    internal static class ExpressionParser
    {
        ...
        private class TermStack
        {
            ...
            public void Add(OperatorCode op)
            {
                ...
                if (state == 1)
                {
                    ...
                    if (waitAfter)
                        reduceUnary();
                    var priority = OperatorManager.GetPriority(op);
                    //直前の計算の優先度が同じか高いなら還元。
            ...
            public IOperandTerm ReduceAll()
            {
                ...
                while (stack.Count > 1)
                    reduceLastThree();
                var retTerm = (IOperandTerm) stack.Pop();
                return retTerm;
            }
            ...
            private void reduceUnary()
            {
                ...
                var operand = (IOperandTerm) stack.Pop();
                var op = (OperatorCode) stack.Pop();
                var newTerm = OperatorMethodManager.ReduceUnaryTerm(op, operand);
                stack.Push(newTerm);
            }
            ...
            private void reduceUnaryAfter()
            {
                ...
                var newTerm = OperatorMethodManager.ReduceUnaryAfterTerm(op, operand);
                stack.Push(newTerm);
            }
            ...
            private void reduceLastThree()
            {
                //if (stack.Count < 2)
                //    throw new ExeEE("不正な時期の呼び出し");
                var right = (IOperandTerm) stack.Pop(); //後から入れたほうが右側
                var op = (OperatorCode) stack.Pop();
                var left = (IOperandTerm) stack.Pop();
                ...
                var newTerm = OperatorMethodManager.ReduceBinaryTerm(op, left, right);
                stack.Push(newTerm);
            }

            private void reduceTernary(IOperandTerm left, IOperandTerm right, OperatorCode op)
            {
                var newOp = (OperatorCode) stack.Pop();
                var newLeft = (IOperandTerm) stack.Pop();

                var newTerm = OperatorMethodManager.ReduceTernaryTerm(newOp, newLeft, left, right);
                stack.Push(newTerm);
            }
        ...
        public static IOperandTerm[] ReduceArguments(WordCollection wc, ArgsEndWith endWith, bool isDefine)
        {
            ...
            var terms = new List<IOperandTerm>();
            ...
            var termEndWith_Assignment = termEndWith | TermEndWith.Assignment;
            while (true)
            {
                var word = wc.Current;
                switch (word.Type)
...
//=がある
                        wc.ShiftNext();
                        var term = reduceTerm(wc, false, termEndWith, VariableCode.__NULL__);
                        if (term == null)
            ...
            var ret = new IOperandTerm[terms.Count];
            terms.CopyTo(ret);
            return ret;
        }
        ...
        {
            var term = reduceTerm(wc, false, endWith, VariableCode.__NULL__);
            return term;
        }
        ...
        {
            var strf = StrForm.FromWordToken(sfw);
            if (strf.IsConst)
        ...
        {
            var terms = new List<CaseExpression>();
            while (!wc.EOL)
            {
                terms.Add(reduceCaseExpression(wc));
                wc.ShiftNext();
            }
            var ret = new CaseExpression[terms.Count];
            terms.CopyTo(ret);
            return ret;
        }
        ...
        {
            var ret = reduceTerm(wc, false, TermEndWith.EoL, varCode);
            if (ret == null)
        ...
        public static VariableToken ReduceVariableIdentifier(WordCollection wc, string idStr)
        {
            ...
            {
                wc.ShiftNext();
                var subidWT = wc.Current as IdentifierWord;
                if (subidWT == null)
...
//関数
                ...
                var mToken =
                    GlobalStatic.IdentifierDictionary.GetFunctionMethod(GlobalStatic.LabelDictionary, idStr, args,
                        false);
                if (mToken == null)
            ...
            //変数 or キーワード
            var id = ReduceVariableIdentifier(wc, idStr);
            ...
            //idStrが変数名でない場合、
            var refToken =
                GlobalStatic.IdentifierDictionary.GetFunctionMethod(GlobalStatic.LabelDictionary, idStr, null, false);
        ...
        private static CaseExpression reduceCaseExpression(WordCollection wc)
        {
            ...
            if (id != null && id.Code.Equals("IS", Config.SCVariable))
            {
                ...
                var type = ret.LeftTerm.GetOperandType();
                return ret;
            }
        ...
        /// <returns></returns>
        private static IOperandTerm reduceTerm(WordCollection wc, bool allowKeywordTo, TermEndWith endWith,
            VariableCode varCode)
        {
            ...
            var varArg = varCode != VariableCode.__NULL__;
            do
            {
                var token = wc.Current;
                switch (token.Type)
                {
                    case '\0':
                        goto end;
                    ...
                    case 'A': //IdentifierWT
                    {
                        var idStr = ((IdentifierWord) token).Code;
                    ...
                    case '=': //OperatorWT
                    {
                        ...
                        var op = ((OperatorWord) token).Code;
                        ...
                        if (formerOp == OperatorCode.Equal || formerOp == OperatorCode.Greater ||
                            formerOp == OperatorCode.Less
                            || formerOp == OperatorCode.GreaterEqual || formerOp == OperatorCode.LessEqual ||
                            formerOp == OperatorCode.NotEqual)
                            if (op == OperatorCode.Equal || op == OperatorCode.Greater || op == OperatorCode.Less
                                || op == OperatorCode.GreaterEqual || op == OperatorCode.LessEqual ||
                                op == OperatorCode.NotEqual)
                                ParserMediator.Warn("（構文上の注意）比較演算子が連続しています。", GlobalStatic.Process.GetScaningLine(), 0,
                                    false, false);
                        stack.Add(op);
                        formerOp = op;
                    ...
                    case '(':
                        wc.ShiftNext();
                        var inTerm = reduceTerm(wc, false, TermEndWith.RightParenthesis, VariableCode.__NULL__);
                        if (inTerm == null)
