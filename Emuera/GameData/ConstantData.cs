...
namespace MinorShift.Emuera.GameData
{
    ...
    internal sealed class ConstantData
    {
        ...
        private readonly List<VariableCode> changedCode = new List<VariableCode>();
        ...
        private readonly Dictionary<string, int>[] nameToIntDics =
            new Dictionary<string, int>[(int) VariableCode.__COUNT_CSV_STRING_ARRAY_1D__];
        ...
        private void setDefaultArrayLength()
        {
            ...
            VariableStrArray2DLength = new long[(int) VariableCode.__COUNT_STRING_ARRAY_2D__];
            VariableIntArray3DLength = new long[(int) VariableCode.__COUNT_INTEGER_ARRAY_3D__];
            VariableStrArray3DLength = new long[(int) VariableCode.__COUNT_STRING_ARRAY_3D__];
            ...
            CharacterStrArray2DLength = new long[(int) VariableCode.__COUNT_CHARACTER_STRING_ARRAY_2D__];
            for (var i = 0; i < VariableIntArrayLength.Length; i++)
                VariableIntArrayLength[i] = 1000;
            VariableIntArrayLength[(int) (VariableCode.__LOWERCASE__ & VariableCode.FLAG)] = 10000;
            VariableIntArrayLength[(int) (VariableCode.__LOWERCASE__ & VariableCode.ITEMPRICE)] =
                MaxDataList[itemIndex];

            VariableIntArrayLength[(int) (VariableCode.__LOWERCASE__ & VariableCode.RANDDATA)] = 625;

            for (var i = 0; i < VariableStrArrayLength.Length; i++)
                VariableStrArrayLength[i] = 100;
            VariableStrArrayLength[(int) (VariableCode.__LOWERCASE__ & VariableCode.STR)] = MaxDataList[strIndex];

            for (var i = 0; i < VariableIntArray2DLength.Length; i++)
                VariableIntArray2DLength[i] = (100L << 32) + 100L;
            for (var i = 0; i < VariableStrArray2DLength.Length; i++)
                VariableStrArray2DLength[i] = (100L << 32) + 100L;

            for (var i = 0; i < VariableIntArray3DLength.Length; i++)
                VariableIntArray3DLength[i] = (100L << 40) + (100L << 20) + 100L;
            for (var i = 0; i < VariableStrArray3DLength.Length; i++)
                VariableStrArray3DLength[i] = (100L << 40) + (100L << 20) + 100L;

            for (var i = 0; i < CharacterIntArrayLength.Length; i++)
            ...
            for (var i = 0; i < CharacterStrArrayLength.Length; i++)
                CharacterStrArrayLength[i] = 100;

            for (var i = 0; i < CharacterIntArray2DLength.Length; i++)
                CharacterIntArray2DLength[i] = (1L << 32) + 1L;
            for (var i = 0; i < CharacterStrArray2DLength.Length; i++)
        ...
        private void changeVariableSizeData(string line, ScriptPosition position)
        {
            ...
            var idtoken = tokens[0].Trim();
            var id = VariableIdentifier.GetVariableId(idtoken);
            if (id == null)
            {
            ...
            {
                ParserMediator.Warn("配列変数でない変数" + id + "のサイズを変更できません", position, 1);
                return;
            }
            if (id.IsCalc || id.Code == VariableCode.RANDDATA)
            {
                ParserMediator.Warn(id + "のサイズは変更できません", position, 1);
                return;
            }
            var length = 0;
            var length2 = 0;
            ...
            if (length <= 0)
            {
                ...
                if (tokens.Length > 2 && tokens[2].Length > 0 && tokens[2].Trim().Length > 0 &&
                    char.IsDigit(tokens[2].Trim()[0]))
            ...
            else if (id.IsArray2D)
            {
                ...
                if (tokens.Length > 3 && tokens[3].Length > 0 && tokens[3].Trim().Length > 0 &&
                    char.IsDigit(tokens[3].Trim()[0]))
            ...
            else if (id.IsArray3D)
            {
                ...
                if (tokens.Length > 4 && tokens[4].Length > 0 && tokens[4].Trim().Length > 0 &&
                    char.IsDigit(tokens[4].Trim()[0]))
            ...
            switch (id.Code)
            {
                ...
                default:
                {
                    if (id.IsCharacterData)
                    {
                        if (id.IsArray2D)
                        {
                            var length64 = ((long) length << 32) + length2;
                            if (id.IsInteger)
                    ...
                    else if (id.IsArray3D)
                    {
                        ...
                        var length3d = ((long) length << 40) + ((long) length2 << 20) + length3;
                        if (id.IsInteger)
        ...
        private void _decideActualArraySize_sub(VariableCode mainCode, VariableCode nameCode, int[] arraylength,
            ScriptPosition position)
        {
            var nameIndex = (int) (nameCode & VariableCode.__LOWERCASE__);
            var mainLengthIndex = (int) (mainCode & VariableCode.__LOWERCASE__);
            if (changedCode.Contains(nameCode) && changedCode.Contains(mainCode))
            {
                if (MaxDataList[nameIndex] != arraylength[mainLengthIndex])
                {
                    var i = Math.Max(MaxDataList[nameIndex], arraylength[mainLengthIndex]);
        ...
        private void decideActualArraySize(ScriptPosition position)
        {
            ...
            _decideActualArraySize_sub(VariableCode.SAVESTR, VariableCode.SAVESTRNAME, VariableStrArrayLength,
                position);
            _decideActualArraySize_sub(VariableCode.GLOBAL, VariableCode.GLOBALNAME, VariableIntArrayLength, position);
            _decideActualArraySize_sub(VariableCode.GLOBALS, VariableCode.GLOBALSNAME, VariableStrArrayLength,
                position);
            ...
            if (changedCode.Contains(VariableCode.PALAMNAME) &&
                (changedCode.Contains(VariableCode.PALAM) || changedCode.Contains(VariableCode.JUEL)))
            {
                var palamJuelMax = Math.Max(
                    CharacterIntArrayLength[(int) (VariableCode.__LOWERCASE__ & VariableCode.PALAM)]
            ...
            if (changedCode.Contains(VariableCode.PALAM) || changedCode.Contains(VariableCode.JUEL))
            {
                ...
                if (changedCode.Contains(VariableCode.PALAMNAME))
                {
                    ...
                    {
                        var i = Math.Max(MaxDataList[paramIndex], palamJuelMax);
                        MaxDataList[paramIndex] = i;
                        if (CharacterIntArrayLength[(int) (VariableCode.__LOWERCASE__ & VariableCode.PALAM)] ==
                            palamJuelMax)
                            CharacterIntArrayLength[(int) (VariableCode.__LOWERCASE__ & VariableCode.PALAM)] = i;
                        if (CharacterIntArrayLength[(int) (VariableCode.__LOWERCASE__ & VariableCode.JUEL)] ==
                            palamJuelMax)
            ...
            else if (changedCode.Contains(VariableCode.PALAMNAME))
            {
                ...
                CharacterIntArrayLength[(int) (VariableCode.__LOWERCASE__ & VariableCode.PALAM)] =
                    MaxDataList[paramIndex];
                //指定のPALAMNAMEがJUELより小さければ警告出してJUELにあわせる
                if (MaxDataList[paramIndex] <
                    CharacterIntArrayLength[(int) (VariableCode.__LOWERCASE__ & VariableCode.JUEL)])
                {
            ...
            //CDFLAG
            //一部変更されたら双方変更されたと扱う
            var cdflagNameLengthChanged = changedCode.Contains(VariableCode.CDFLAGNAME1) ||
                                          changedCode.Contains(VariableCode.CDFLAGNAME2);
            var mainLengthIndex = (int) (VariableCode.__LOWERCASE__ & VariableCode.CDFLAG);
            var length64 = CharacterIntArray2DLength[mainLengthIndex];
            var length1 = (int) (length64 >> 32);
        ...
        public void LoadData(string csvDir, EmueraConsole console, bool disp)
        {
            ...
            //逆引き辞書を作成2 (RELATION)
            for (var i = 0; i < CharacterTmplList.Count; i++)
            {
                var tmpl = CharacterTmplList[i];
        ...
        public int KeywordToInteger(VariableCode code, string key, int index)
        {
            ...
            var ret = -1;
            string errPos;
            var dic = GetKeywordDictionary(out errPos, code, index);
        ...
        {
            foreach (var chara in CharacterTmplList)
                if (chara.No == index)
                    return chara;
            return null;
        }
        ...
        private void loadCharacterData(string csvDir, bool disp)
        {
            if (!Directory.Exists(csvDir))
                return;
            var csvPaths = Config.GetFiles(csvDir, "CHARA*.CSV");
            for (var i = 0; i < csvPaths.Count; i++)
                loadCharacterDataFile(csvPaths[i].Value, csvPaths[i].Key, disp);
            if (useCompatiName)
                foreach (var tmpl in CharacterTmplList)
                    if (string.IsNullOrEmpty(tmpl.Callname))
                        tmpl.Callname = tmpl.Name;
            foreach (var tmpl in CharacterTmplList)
                tmpl.SetSpFlag();
            var nList = new Dictionary<long, CharacterTemplate>();
            var spList = new Dictionary<long, CharacterTemplate>();
            foreach (var tmpl in CharacterTmplList)
            {
                var targetList = nList;
                ...
                if (targetList.ContainsKey(tmpl.No))
                    if (!Config.CompatiSPChara && tmpl.IsSpchara != targetList[tmpl.No].IsSpchara)
                        ParserMediator.Warn(
                            "番号" + tmpl.No + "のキャラが複数回定義されています(SPキャラとして定義するには互換性オプション「SPキャラを使用する」をONにしてください)", null, 1);
                    else
                        ParserMediator.Warn("番号" + tmpl.No + "のキャラが複数回定義されています", null, 1);
                else
            ...
            try
            {
                ...
                while ((st = eReader.ReadEnabledLine()) != null)
                {
                        ...
                        || tokens[0].Equals("番号", Config.SCVariable))
                    {
                        ...
                        var no = eReader.Filename.ToUpper();
                        no = no.Substring(no.IndexOf("CHARA") + 5);
                        var sb = new StringBuilder();
                        var ss = new StringStream(no);
        ...
        private void toCharacterTemplate(GameBase gamebase, ScriptPosition position, CharacterTemplate chara,
            string[] tokens)
        {
            if (chara == null)
                return;
            ...
            var p1isNumeric = tryToInt64(tokens[1].TrimEnd(), out p1);
            if (p1isNumeric && (p1 < 0 || p1 >= length))
            {
                ParserMediator.Warn(p1 + "は配列の範囲外です", position, 1);
                return;
            }
            var index = (int) p1;
            ...
            try
            {
                ...
                while ((st = eReader.ReadEnabledLine()) != null)
                {
                    ...
                    if (targetI != null && tokens.Length >= 3)
                    {
                        ...
                        if (!long.TryParse(tokens[2].TrimEnd(), out price))
                        {
    ...
    internal sealed class CharacterTemplate
    {
        ...
        public int ArrayLength(CharacterIntData type)
        {
            switch (type)
            {
                ...
                {
                    var size = arraySize[(int) (VariableCode.__LOWERCASE__ & VariableCode.BASE)];
                    var maxSize = arraySize[(int) (VariableCode.__LOWERCASE__ & VariableCode.MAXBASE)];
