...
namespace MinorShift.Emuera
{
    ...
    {
        private static readonly string configPath = Program.ExeDir + "emuera.config";
        private static readonly string configdebugPath = Program.DebugDir + "debug.config";

        //適当に大き目の配列を作っておく。
        private readonly AConfigItem[] configArray = new AConfigItem[70];

        private readonly AConfigItem[] debugArray = new AConfigItem[20];
        private readonly AConfigItem[] replaceArray = new AConfigItem[50];
        ...
        private void setDefault()
        {
            ...
            configArray[i++] =
                new ConfigItem<TextDrawingMode>(ConfigCode.TextDrawingMode, "描画インターフェース", TextDrawingMode.GRAPHICS);
            ...
            configArray[i++] =
                new ConfigItem<Color>(ConfigCode.ForeColor, "文字色", Color.FromArgb(192, 192, 192)); //LIGHTGRAY
            configArray[i++] = new ConfigItem<Color>(ConfigCode.BackColor, "背景色", Color.FromArgb(0, 0, 0)); //BLACK
            configArray[i++] =
                new ConfigItem<Color>(ConfigCode.FocusColor, "選択中文字色", Color.FromArgb(255, 255, 0)); //YELLOW
            configArray[i++] =
                new ConfigItem<Color>(ConfigCode.LogColor, "履歴文字色",
                    Color.FromArgb(192, 192, 192)); //LIGHTGRAY//Color.FromArgb(128, 128, 128);//GRAY
            ...
            configArray[i++] = new ConfigItem<ReduceArgumentOnLoadFlag>(ConfigCode.ReduceArgumentOnLoad, "ロード時に引数を解析する",
                ReduceArgumentOnLoadFlag.NO);
            ...
            configArray[i++] = new ConfigItem<DisplayWarningFlag>(ConfigCode.FunctionNotFoundWarning, "関数が見つからない警告の扱い",
                DisplayWarningFlag.IGNORE);
            configArray[i++] = new ConfigItem<DisplayWarningFlag>(ConfigCode.FunctionNotCalledWarning,
                "関数が呼び出されなかった警告の扱い", DisplayWarningFlag.IGNORE);
            //configArray[i++] = new ConfigItem<List<string>>(ConfigCode.IgnoreWarningFiles, "指定したファイル中の警告を無視する", new List<string>());
            configArray[i++] =
                new ConfigItem<bool>(ConfigCode.ChangeMasterNameIfDebug, "デバッグコマンドを使用した時にMASTERの名前を変更する", true);
            ...
            configArray[i++] = new ConfigItem<TextEditorType>(ConfigCode.EditorType, "テキストエディタコマンドライン指定",
                TextEditorType.USER_SETTING);
            configArray[i++] = new ConfigItem<string>(ConfigCode.EditorArgument, "エディタに渡す行指定引数", "");
            configArray[i++] = new ConfigItem<bool>(ConfigCode.WarnNormalFunctionOverloading,
                "同名の非イベント関数が複数定義されたとき警告する", false);
            ...
            configArray[i++] =
                new ConfigItem<bool>(ConfigCode.CompatiFunctionNoignoreCase, "関数・属性については大文字小文字を無視しない", false);
            ;
            ...
            configArray[i++] =
                new ConfigItem<UseLanguage>(ConfigCode.useLanguage, "内部で使用する東アジア言語", UseLanguage.JAPANESE);
            configArray[i++] = new ConfigItem<bool>(ConfigCode.AllowLongInputByMouse, "ONEINPUT系命令でマウスによる2文字以上の入力を許可する",
                false);
            ...
            configArray[i++] =
                new ConfigItem<bool>(ConfigCode.CompatiFuncArgAutoConvert, "ユーザー関数の引数に自動的にTOSTRを補完する", false);
            configArray[i++] = new ConfigItem<bool>(ConfigCode.SystemIgnoreTripleSymbol, "FORM中の三連記号を展開しない", false);
            configArray[i++] =
                new ConfigItem<bool>(ConfigCode.TimesNotRigorousCalculation, "TIMESの計算をeramakerにあわせる", false);
            //一文字変数の禁止オプションを考えた名残
            ...
            replaceArray[i++] = new ConfigItem<List<long>>(ConfigCode.StainDefault, "汚れの初期値",
                new List<long>(new long[] {0, 0, 2, 1, 8}));
            replaceArray[i++] = new ConfigItem<string>(ConfigCode.TimeupLabel, "時間切れ表示", "時間切れ");
            replaceArray[i++] = new ConfigItem<List<long>>(ConfigCode.ExpLvDef, "EXPLVの初期値",
                new List<long>(new long[] {0, 1, 4, 20, 50, 200}));
            replaceArray[i++] = new ConfigItem<List<long>>(ConfigCode.PalamLvDef, "PALAMLVの初期値",
                new List<long>(new long[] {0, 100, 500, 3000, 10000, 30000, 60000, 100000, 150000, 250000}));
        ...
        {
            var ret = new Dictionary<ConfigCode, string>();
        ...
        {
            foreach (var item in replaceArray)
            {
                if (item == null)
                    continue;
                if (item.Code == code)
        ...
        {
            foreach (var item in debugArray)
            {
                if (item == null)
                    continue;
                if (item.Code == code)
        ...
        {
            var item = Instance.GetItem(text);
            if (item == null)
            {
