...
namespace MinorShift.Emuera
{
    internal static class Config
    {
        ...
        public static void SetConfig(ConfigData instance)
        {
            ...
            var lang = instance.GetConfigValue<UseLanguage>(ConfigCode.useLanguage);
            switch (lang)
            {
        ...
        private static readonly Dictionary<string, Dictionary<FontStyle, Font>> fontDic =
            new Dictionary<string, Dictionary<FontStyle, Font>>();
        ...
        private static void createSavDirAndMoveFiles()
        {
            ...
            var existGlobal = File.Exists(Program.ExeDir + "global.sav");
            var savFiles = Directory.GetFiles(Program.ExeDir, "save*.sav", SearchOption.TopDirectoryOnly);
            ...
            try
            {
                ...
                foreach (var oldpath in savFiles)
        ...
        private static long getUpdateKey()
        {
            ...
            for (var i = 0; i < erbFiles.Length; i++)
            ...
            long key = 0;
            for (var i = 0; i < writetimes.Length; i++)
                unchecked
                {
        ...
        private static List<KeyValuePair<string, string>> getFiles(string dir, string rootdir, string pattern,
            bool toponly, bool sort)
        {
            ...
            {
//サブフォルダ内の検索
                var dirList = Directory.GetDirectories(dir, "*", SearchOption.TopDirectoryOnly);
                if (dirList.Length > 0)
                {
                    if (sort)
            ...
            //filepathsは完全パスである
            var filepaths = Directory.GetFiles(dir, pattern, SearchOption.TopDirectoryOnly);
            if (sort)
                Array.Sort(filepaths, ignoreCaseComparer);
            for (var i = 0; i < filepaths.Length; i++)
                if (Path.GetExtension(filepaths[i]).Length <= 4) //".erb"や".csv"であること。放置すると".erb*"等を拾う。
                    retList.Add(new KeyValuePair<string, string>(RelativePath + Path.GetFileName(filepaths[i]),
                        filepaths[i]));
            return retList;
        }
